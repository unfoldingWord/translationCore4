...
- **Conformance proof:** `cd conformance && npm install && npm run generate && npm run validate` (expect <!-- manifest: conformance:validate passed -->47 passed, 0 failed — Stage-1 <!-- manifest: conformance:validate summary[Stage-1] -->36, Stage-2 <!-- manifest: conformance:validate summary[Stage-2] -->2, Phase-2 <!-- manifest: conformance:validate summary[Phase-2] -->3, OBS <!-- manifest: conformance:validate summary[OBS] -->6 — the OBS group runs against `sample-burrito-obs/`, BURRITO-SPEC §10). Journal suite: `npm run validate:journal` (expect <!-- manifest: conformance:journal passed -->350 passed, 0 failed). Normative gate: `npm run validate:normative` (expect <!-- manifest: conformance:normative passed -->90 rules, 0 uncovered; §8 and §10). All three: `validate:all`. [VERIFIED — the counts are read from `docs/evidence/manifest.json`, written by `npm run prove` in the CI run; that file names the run's commit, date and Node version; `npm run docs:gate` fails when this line disagrees with it (L-2, #155). The earlier counts, 34/34 and 59/59 at 4afd5b0 (2026-08-18) and 33/33, 31/31, 30/30 at the 1.6-, 1.5- and 1.4-draft change sets, are history in git.]
