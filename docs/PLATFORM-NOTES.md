...
34. **The word-aligner fork's in-memory contract, and where tC4's data meets it** [VERIFIED — `evidence/word-aligner-rcl-contract-2026-09-05.md`: `@gabrielaillet/word-aligner-rcl` 1.3.18 (tag v1.3.18 = f0f4326), source read via the published source map, 2026-09-05; closes issue [#13](https://github.com/unfoldingWord/translationCore4/issues/13)]. tC4 does not import the fork. It is a `devDependency`, and the alignment screen reads the §5.1 record directly (A-5). The fork's `WordAligner` and `SuggestingWordAligner` require `verseAlignments` (`{sourceNgram, targetNgram}` of words `{index, text, occurrence, occurrences, strong?, lemma?, morph?}`) and `targetWords` (every target token of the verse, with `disabled`). `occurrence`/`occurrences` are numbers (strings are `parseInt`ed). `index` is the 0-based token position and drives all sorting. The suggester receives `wordmap-lexer` `Token`s built from these words, with sequential positions assigned before the call. tC4's §5.1 record is the same word data under `topWords`/`bottomWords`/`word`, coerced to numbers at the store boundary (I-2). It holds no token `index`. Its `wordBank` is the unaligned subset; the fork's `targetWords` is the whole set, aligned words marked `disabled`. Both derive from the verse text plus the record. Unmeasured: whether tC4's regex tokenizer and `wordmap-lexer`'s `string-punctuation-tokenizer` 2.0.0 split a verse the same way. The record also carries the owner's 2026-08-13 findings on gatewayEdit and Bible Editor.
