...
9. **Nothing auto-commits. `add-and-commit` sweeps the whole repo. Branch operations refuse on dirty trees.** Commit at checkpoints, and always commit before branch operations (BURRITO-SPEC W-4). `add-and-commit` on a CLEAN tree also succeeds and records an EMPTY commit, so a checkpoint reads `GET /git/status/<repo>` first (a list of `{path, change_type}`; empty means clean) and commits only when the list is not empty [VERIFIED — pankosmia-web 0.18.5 (99fd9be, 2026-07-30), read 2026-09-05: `src/endpoints/git2/status.rs:25` mounts `GET /status/<repo_path..>`, `:48` drops `Status::CURRENT` entries, `:82-85` emits `{path, change_type}` records; live on the rig the same day, `GET /api/git/status/_local_/_local_/sample_burrito` returned that array for a dirty tree, and an `add-and-commit` with `git status --porcelain` empty advanced `git rev-list --count HEAD` from 2 to 3 with the new message at `git log -1`; issue #183].
...
10. **Flavor→client routing:** the content grid uses only `editTable[flavor][0]`. When both are bundled, one client shadows the others. tC4's own product bundle omits the content and workspace clients, so it never enters this contest. (The prototype `desktop-app-tc4` DOES bundle both, at CLIENT2 and CLIENT11.)
    ...
    · `[PLATFORM]` — re-verified 2026-08-26 at the current versions (issue [#31](https://github.com/unfoldingWord/translationCore4/issues/31) claim (a), final). Server: `src/endpoints/clients.rs` still builds the response as a `BTreeMap` keyed by client id (`:50`, insert `:97`) [VERIFIED — pankosmia-web 0.18.7 (c43c40d, 2026-08-11)]; the two commits since the 0.18.5 pin touch offline-start logic only. Client: `core-client-content` `main` is still EXACTLY the cited revision (e504268, 2026-04-10) — the cited code is current by identity. The whole chain holds unchanged.
...
17. **The public `pull-repo` endpoint has no branch parameter.** It fetches with an empty refspec, and it merges the target that `FETCH_HEAD` resolves to. Ref-ordering is not a safe contract (probe, 2026-07-10). OPEN-QUESTIONS #23 is CLOSED (2026-07-18): one single-branch publication repo per actor makes `pull-repo` deterministic — one head, so no incorrect head can be selected [VERIFIED — transport rig 10/10, `evidence/transport-rig-2026-07-18.md`]. Multi-branch `pull-repo` stays ordering-steered and unsafe.
    ...
    · `[PLATFORM]` — re-verified 2026-08-26 at 0.18.7 (c43c40d, 2026-08-11): the route is `POST /api/git/pull-repo/<remote_name>/<repo_path..>` (`pull_repo.rs:77`; mount `/api/git`, `src/utils/launch.rs:100`) — a remote NAME, still no branch parameter — and `:123-129` still resolves `FETCH_HEAD` into `merge_analysis`. See note #33 for the full merge-surface inventory (issue #31 claim (b)).
...
32. **The desktop template ships Electronite, not plain Electron.** [VERIFIED — desktop-app-template 4cb7576 (2026-08-14)] The wrapper answer for issue [#32](https://github.com/unfoldingWord/translationCore4/issues/32) and D20: the template's install scripts download prebuilt **Electronite v37.1.0-graphite** binaries from `github.com/unfoldingWord/electronite` releases, for macOS, Linux and Windows, arm64 and x64 (`macos/install/makeAllInstallsElectronite.sh:56-57`, `linux/install/makeAllInstallsElectronite.bsh:56-57`, `windows/install/makeAllInstallsElectronite.ps1:51`). No substitution is needed. There is no Electron npm dependency at all — the app dir carries only `electronStartup.js` plus a small node_modules subset, and the Electronite binary loads it directly. The template's `local_server` pins `pankosmia_web = "=0.16.20"` on `main` (dev tier 0.18.7 via `local_server.env`), which differs from our 0.18.5 rev pin — a tC4 package must build the server from `dev-env/server` instead. Witnessed 2026-08-14 on macOS arm64: an Electronite `v37.1.0-graphite` window (CDP reports Chrome/138.0.7204.35) booted into `/clients/uw-tc4` and rendered the tC4 project list against the 0.18.5 rig. Graphite font shaping in the packaged app is PROVEN [VERIFIED — 2026-08-14, macOS arm64]: the Graphite-only Awami Nastaliq (served by the app's own `/api/webfonts` route) shapes correctly in the packaged Electronite window (measureText width 390.06px, joined stacked Nastaliq), and the identical probe in plain Chromium 151 renders unshaped isolated glyphs (770.32px) — same font, server, and text, so the difference is Graphite. Method, measurements and screenshots: `docs/evidence/graphite-shaping-packaged-2026-08-14.md`. D20 is satisfied end to end.

33. **The merge capability of the HTTP API, stated exactly** [VERIFIED — pankosmia-web 0.18.7 (c43c40d, 2026-08-11); closes issue [#31](https://github.com/unfoldingWord/translationCore4/issues/31) claim (b)]. The complete `git2` endpoint inventory (`src/endpoints/git2/`, mounted at `/api/git`, `src/utils/launch.rs:100`) contains exactly ONE endpoint that performs a merge: `POST /api/git/pull-repo/<remote_name>/<repo_path..>` — a fetch from the NAMED remote followed by a merge of whatever `FETCH_HEAD` resolves to (note #17; no branch parameter, and note #21's worktree hazard applies to its normal-merge path). **No endpoint merges two local branches.** The branch endpoints (`branches.rs`, `set_branch.rs`, `create_and_set_branch.rs`) list, switch and create — never merge. The way the platform's own workflows merge across LOCAL repositories is itself an HTTP surface, not an internal shortcut: `POST /api/git/remote/add/<repo_path..>?remote_name&remote_url` accepts a `file://` transport explicitly (`add_remote.rs:44`, transport regex `^(https?|ssh|file)://|git@`, with a three-part repo-path shorthand as the fallback), so a client merges local-to-local by adding a `file://` remote and then calling `pull-repo` on it — the mechanism tC4's transport suite exercises. Any future statement of the form "the platform can/cannot merge X" must name one of these two routes.
    · `[PLATFORM]` — source read at desktop-app-template 4cb7576 (2026-08-14) + live boot. See `docs/PACKAGING.md`.

34. **The word-aligner fork's in-memory contract, and where tC4's data meets it** [VERIFIED — `evidence/word-aligner-rcl-contract-2026-09-05.md`: `@gabrielaillet/word-aligner-rcl` 1.3.18 (tag v1.3.18 = f0f4326), source read via the published source map, 2026-09-05; closes issue [#13](https://github.com/unfoldingWord/translationCore4/issues/13)]. tC4 does not import the fork. It is a `devDependency`, and the alignment screen reads the §5.1 record directly (A-5). The fork's `WordAligner` and `SuggestingWordAligner` require `verseAlignments` (`{sourceNgram, targetNgram}` of words `{index, text, occurrence, occurrences}`, source words also `{strong, lemma, morph}` typed as required strings), `targetWords` (every target token of the verse, with `disabled`), and a `contextId` with `reference.chapter` and `reference.verse` (the grid renders empty without it). `occurrence`/`occurrences` are numbers (strings are `parseInt`ed). `index` is the 0-based token position and drives all sorting. The suggester receives `wordmap-lexer` `Token`s built from these words, with sequential positions assigned before the call. tC4's §5.1 record is the same word data under `topWords`/`bottomWords`/`word`, coerced with `Number()` at the store boundary (I-2). It holds no token `index` and no aligner `contextId`; it types `strong`/`lemma`/`morph` as optional. Its `wordBank` is the unaligned subset; the fork's `targetWords` is the whole set, aligned words marked `disabled`. All of these derive from the verse text plus the record. Unmeasured: whether tC4's regex tokenizer and `wordmap-lexer`'s `string-punctuation-tokenizer` 2.0.0 split a verse the same way. The owner's 2026-08-13 findings on gatewayEdit and Bible Editor stay on issue #13; the record links them.
    · `[LIBRARY]` — a component library contract, not platform behavior. Read at the pinned version; the fork's repository is `gabrielaillet/word-aligner-rcl-pankosmia`.

35. **DCS catalog labels and sb-zip export labels differ for OBS help resources.** [VERIFIED — DCS production `/api/v1/catalog/search` and `/sb/<tag>.zip` `metadata.json`, pinned OBS tags and commits below, read 2026-09-16]. The export metadata is the recorded pin flavor (D58): `unfoldingWord/en_obs` v9 (`d39a1dc7a7557ac54e4a8fecc3462147fe7eec3b`) declares `gloss/textStories`; `unfoldingWord/en_obs-tn` v13 (`e86138ea13f619f09f7a6dcaa60592716d407fe4`) declares `peripheral/x-obsnotes`; `unfoldingWord/en_obs-twl` v3 (`44ebc9fafe8101665f985007d566f5036a2be85b`) declares `parascriptural/x-bcvarticles`; and `uW/obs_images_360` at `7146d5b504f6b63b9e11f7dc0b18c594d0ae179d` declares `peripheral/x-obsimages`. The catalog currently reports aliases for the two help repos (`parascriptural/x-notes` and `parascriptural/x-links`). tC4 uses the catalog only to discover candidate repos and uses the repo-name suffix for OBS role assignment; installed and project pins retain the export's recorded flavor.
    · `[EXTERNAL]` — DCS / Door43 behavior, not Pankosmia. The export and catalog surfaces were read at the pinned resources and dated above.

36. **`new-obs-resource` writes a `metadata.json` the server cannot parse back.** [VERIFIED — pankosmia-web 0.18.5 (99fd9be, 2026-07-30): `src/endpoints/git2/new_obs_resource.rs` stamps `metadata.json` from `templates/content_templates/text_stories/metadata.json` by string replacement; `src/structs.rs` `BurritoMetadata` requires `localizedNames` with no serde default; live rig 2026-09-15]. The `text_stories` template of `resource-core` 54802be has no `localizedNames`, the `text_translation` template has `"localizedNames": {}`. So on a project the OBS route created, every `update_ingredients` write and every `remake-ingredients` answers `Could not parse metadata: missing field \`localizedNames\``; a plain write succeeds; a project the Bible route created is fine. tC4's fix (#287): `scripts/fix-obs-template.mjs` adds the key to the template wherever tC4 serves it — the pins rig route (`dev-env/scripts/setup-from-pins.zsh`, the clean-clone path and the `rig` CI job), the assembled-build rig route (`dev-env/scripts/setup.zsh`), and the packaged installer (`scripts/package-desktop.zsh`) — so the project the platform creates is one its own struct parses. The vendored conformance fixture stays byte-for-byte; the sample carries the key. Because of #37 the app never triggers that parse on an OBS project today; the fix keeps the created metadata honest for the day it does. Upstream ask, routed through the owner: add `localizedNames` to the `text_stories` template, or make the field optional in the struct.
    · `[PLATFORM]` — general, not a product configuration: the struct and the template are the server's own.

37. **A registering write or a rescan EMPTIES an OBS project's `currentScope`.** [VERIFIED — pankosmia-web 0.18.5 (99fd9be, 2026-07-30): `post_raw_ingredient.rs` with `update_ingredients` and `post_remake_ingredients_metadata.rs` both replace `type.flavorType.currentScope` with `ingredients_scopes_from_files(...)`, the scope of the USFM files on disk; live rig 2026-09-16: a project created by `new-obs-resource` has the template's 33-book table at its initial commit and an empty table after one registering write]. An OBS project has no USFM, so the platform's rebuild is `{}` — while BURRITO-SPEC R-10.2.3 keeps the template's table verbatim and no HTTP route writes `metadata.json` (D28). tC4's rule (#287, `JournalingStore.registerIngredients`): on an OBS project every derived write is a plain write and a checkpoint does not rescan, so the table the platform stamped at creation stands. The cost: the metadata `ingredients` table of an OBS project is the platform's from creation (the template titles' checksums; the `checking/` sidecars unlisted) — the app reads paths from `/burrito/paths`, never from that table, and the recorded md5 was already documented as non-authoritative (`readBook`). Upstream ask, routed through the owner: preserve `currentScope` when the flavor is not scripture. Until then, do not run `remake-ingredients` by hand on an OBS project.
    · `[PLATFORM]` — general: both endpoints, one helper.

38. **A directory removed during an ingredient rescan panics the server and poisons it: every later request returns HTTP 500 until a restart.** [VERIFIED — pankosmia-web 0.18.5 (99fd9be, 2026-07-30): `src/utils/burrito.rs:104` `entry.unwrap()` inside `for entry in WalkDir::new(&repo_path)` in `ingredients_metadata_from_files`; still present at upstream `origin/main` 0.18.9 (0fd1c2a, 2026-09-18), same line; 122 `lock().unwrap()` call sites in `src/` at both revisions. Observed 2026-09-17 on the rig during the journey run of PR [#314](https://github.com/unfoldingWord/translationCore4/pull/314): 15 failures with 500 from `GET /burrito/metadata/summaries`, server log a panic at `burrito.rs:104` during `post_raw_ingredient` on `sample_burrito` while `ingredients/checking/alignments` was absent. Issue [#315](https://github.com/unfoldingWord/translationCore4/issues/315).] A registering write (`update_ingredients`) and the full rescan both walk the whole repo tree. A `WalkDir` item is a `Result`; when a directory disappears or cannot be read during the walk, the item is an `Err` and `unwrap()` panics. The panic happens while a state mutex is held, so the lock is poisoned and every later handler that takes it panics on the `PoisonError` and answers 500. Only a restart clears it. Exposure: the app never removes a directory from an open project (its deletes are one file to `.bak`, and the repo of a create that failed); the trigger on the rig is the journey harness removing `checking/` between specs while a debounced save from the previous spec still lands; on a pilot's machine only a person or a sync tool deleting folders under the project store during a save can cause it. Rulings (owner, 2026-09-18, issue #315): the journey harness probes `GET /burrito/metadata/summaries` in `global-setup` and fails fast with the cause when the rig is poisoned (its own issue); the rig scripts do not restart the server between runs; the observation goes upstream as a short robustness report from the project owner.
    · `[PLATFORM]` — general: one helper, two endpoints (`post_raw_ingredient.rs` with `update_ingredients`, `post_remake_ingredients_metadata.rs`); source read at the pin and at upstream HEAD, dated above.

39. **The server selects its resources through TWO selectors: the launch `APP_RESOURCES_DIR`, and from the second start on the `app_resources_dir` saved in `user_settings.json`. `new-obs-resource` copies its story template from the saved one.** [VERIFIED — pankosmia-web 0.18.5 (99fd9be, 2026-07-30): `src/utils/bootstrap.rs:48-68` copies `templates/user_settings.json` from the launch directory into the working dir on the first start; `src/lib.rs:149-167` reads `app_resources_dir` from the saved `user_settings.json` on every later start and panics when it is absent or not a directory; `src/endpoints/git2/new_obs_resource.rs:118-137` copies `content_templates/text_stories/ingredients` from `state.app_resources_dir`. desktop-app-template 4cb7576 (2026-08-14): `buildResources/electron/electronStartup.js:46-51` forwards the parent process's `APP_RESOURCES_DIR` to the server when it is set, `./lib/` otherwise. Source read 2026-09-21. At upstream `origin/main` 0.18.10 (84c322a, 2026-09-18) `src/lib.rs` is restructured around a launch-config key `app_resources_path`; not re-verified there.] Consequences, reproduced 2026-09-19 against the pinned dev server and the packaged macOS server (issues [#347](https://github.com/unfoldingWord/translationCore4/issues/347), [#348](https://github.com/unfoldingWord/translationCore4/issues/348)): (1) a parent shell that still carries `APP_RESOURCES_DIR` from an older unpack makes a new `server.exe` answer the old `product.json` on `/api/version` and copy the old story template; (2) after the launch variable is corrected, a profile whose `user_settings.json` was written under the old tree still serves the old template, so `/api/version` is right while the stories are old; (3) a fresh HOME does not clear an inherited process variable, and a matching executable hash does not prove matching resources. [REPORTED — the owner's Windows transcript of 2026-09-19 on DESKTOP-SMQLQM3, Windows 11 10.0.26200, alpha.7 zip of run 35413933806: the process-level `APP_RESOURCES_DIR` named an alpha.6 `lib\`, `/api/version` said alpha.6, the created `01.md` was 1225 bytes with 68 CR against the install's 1157-byte LF template.] tC4's fix (#348, PR [#354](https://github.com/unfoldingWord/translationCore4/pull/354), merged 2026-09-21 as 552b735): the tracked Electron entry point `scripts/desktop-main.cjs`, under the singleton lock and before `electronStartup.js` captures its environment, sets `APP_RESOURCES_DIR` to this package's absolute `lib/`; when a `user_settings.json` exists under the product's profile, it rewrites only `app_resources_dir` in it through an atomic temp-file publish; malformed settings fail explicitly and stay untouched; `START_SERVER=false` and the dev rig are left alone. The build-time and installed smokes launch every artifact with a deliberately poisoned parent variable and, after a restart, with a profile pointed at the poisoned tree; both selectors must be rebound and a fresh OBS project must serve all 50 template bytes before any client write. Proven on GitHub runners at 552b735 (run 35643034107, 2026-09-21: ubuntu-24.04, macOS arm64 in three install forms, windows-2025 with an install path that contains spaces and no development tools on PATH) — `docs/evidence/packaged-resource-binding-2026-09-21.md`. Distinction from #306: #306 (PR [#343](https://github.com/unfoldingWord/translationCore4/pull/343)) was the rig's upstream checkouts converting the story files to CRLF on Windows, fixed by enforcing LF in the clones and validated at assembly by `scripts/fix-obs-template.mjs` (#344); this note is the server selecting and persisting an old resource tree, unrelated to checkout newline settings. [VERIFIED — retest on the affected machine, DESKTOP-SMQLQM3 (Windows 11 10.0.26200, account `tc-win-test`), 2026-09-21, build 552b735 of run 35643034107 (`built_utc` 2026-09-21T19:13:06Z), with the poisoned process `APP_RESOURCES_DIR` kept set throughout; transcript in [#347](https://github.com/unfoldingWord/translationCore4/issues/347#issuecomment-5766881234). The alpha.6 tree's own `01.md` is 1225 bytes with 68 CR, the same bytes as the 2026-09-19 failing project: the mechanism is confirmed, the old template was copied, nothing converted it. Negative control, the 2026-09-19 unpack under a fresh profile: `/api/version` alpha.6 dated 2026-09-14T14:57:56Z, new story 1225 bytes with 68 CR on disk and over HTTP. New build, fresh profile: `app_resources_dir` written as `C:\Users\tc-win-test\tc4-a8\app\lib\`, alpha.7 dated 2026-09-21T19:13:06Z, new story 1157 bytes with 0 CR on disk and over HTTP. New build, the contaminated 2026-09-19 profile `home-obs-probe347`: `app_resources_dir` read `…\tc4-a6\app\translationCore4\lib\` before launch and `…\tc4-a8\app\lib\` after, version and story bytes as in the fresh run.]
    · `[PLATFORM]` — general: the two selectors and the template copy are the server's own; the forwarding is the template launcher's. Source read at the pin, dated above.

## Verification recipes

- **Conformance proof:** `cd conformance && npm install && npm run generate && npm run validate` (expect <!-- manifest: conformance:validate passed -->48 passed, 0 failed — Stage-1 <!-- manifest: conformance:validate summary[Stage-1] -->36, Stage-2 <!-- manifest: conformance:validate summary[Stage-2] -->2, Phase-2 <!-- manifest: conformance:validate summary[Phase-2] -->3, OBS <!-- manifest: conformance:validate summary[OBS] -->7 — the OBS group runs against `sample-burrito-obs/`, BURRITO-SPEC §10). Journal suite: `npm run validate:journal` (expect <!-- manifest: conformance:journal passed -->350 passed, 0 failed). Normative gate: `npm run validate:normative` (expect <!-- manifest: conformance:normative passed -->91 rules, 0 uncovered; §8 and §10). All three: `validate:all`. [VERIFIED — the counts are read from `docs/evidence/manifest.json`, written by `npm run prove` in the CI run; that file names the run's commit, date and Node version; `npm run docs:gate` fails when this line disagrees with it (L-2, #155). The earlier counts, 34/34 and 59/59 at 4afd5b0 (2026-08-18) and 33/33, 31/31, 30/30 at the 1.6-, 1.5- and 1.4-draft change sets, are history in git.]
- **Editor data-loss repro:** `cd sample-burrito-validation/zaln-strip-repro && npm install && node test.mjs` (expect zero `\zaln`/`\w` in output). (Maintainer workspace — this tool is not in this repository.)
- **Metadata-drop repro:** `cd sample-burrito-validation/metadata-drop-repro && cargo run -- ../../sample-burrito/metadata.json` (expect both drops reported). (Maintainer workspace — this tool is not in this repository.)
