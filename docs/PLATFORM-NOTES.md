...
10. **Flavor→client routing:** the content grid uses only `editTable[flavor][0]`. When both are bundled, `core-local-workspace` shadows the later clients. tC4's own product bundle omits the content and workspace clients, so it never enters this contest. (The prototype `desktop-app-tc4` DOES bundle both, at CLIENT2 and CLIENT11.) (The ordering mechanism has an open re-verification item — OPEN-QUESTIONS #22.)
    · `[PLATFORM]` — re-verified 2026-08-04 in `core-client-content/src/components/DataGridComponent.jsx:280-281` at HEAD (pushed 2026-07-31). Generality confirmed: `core-client-workspace` (declared id `core-local-workspace`) and `core-contenthandler_text_translation` both claim the `textTranslation` flavor, in **all three** product rosters — so this is general, not a tC4 configuration effect. Roster order decides which one wins.
...
25. **Register a client at `/clients/<product.homepage>`, or the 0.18.x server panics at startup.** Since 0.18.0 ("No more main"), the server panics when no client is registered at `/clients/<product.homepage>`. The default `homepage` is `dashboard`, but the bundled dashboard client declares `/clients/main`. Thus the rig's `product.json` sets `"homepage": "main"` [VERIFIED — rig re-baseline at pkg_version 0.18.3, 2026-07-30; STATE.md D27(a)]. Set `homepage` to the tC4 client when tC4 becomes the landing client. That closes OPEN-QUESTIONS #4.
