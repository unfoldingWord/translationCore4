...
| `src/data/import/tc3.ts` | parser `tc3`: one or more tC3 project `.zip` files → one bundle, one book for each zip; the current state only, no history conversion (D80 point 3). The text is tC3's own USFM export of the chapter JSON (`setUpUSFMJSONObject` + `toUSFM` with `forcedNewLines`), with the stored `headers.json` and `\usfm 3.0`. The §5.1 alignments come from `apps/translationCore/alignmentData/`, NFC (I-4), integer occurrences (I-2), in the `unmerge` shape; `wordAlignments/<project>.usfm` is only the cross-check, because tC3's aligned export drops words inside nested character markers (28 JHN and 23 JOB verses of the KJV fixtures). The §5.2 decisions are the check index items that hold user data, with the category from `categoryIndex/` and the time of the latest `checkData/` record. The review facts are the manifest's language, license and contributors. The bundle carries its checking files (`sidecars`) and its resource versions (`versions`); the review step resolves them with `resolveVersions` and `applyVersions` (D82): a version DCS has is a full pin, else the user goes online or uses the installed pins, and the decisions carry over (D36). Several zips must agree on the language (else a damaged finding); a license difference is chosen on the review page. No `manifest.json` gives `import.damaged.no-manifest` | https://github.com/unfoldingWord/translationCore4/issues/21 | none; the version lookup reads the DCS tags API | `test/import/tc3.test.ts` (golden); `fixtures/import/tc3/` (`cfm_fbt_tit_book.zip`, `multi/` JHN JOB LUK, `no-manifest/`); manifest field `license` | `j09` tC3 block (offline with installed versions, online with the guided fix, the multi-zip set) |
