...
**Companions:** `BURRITO-SPEC.md` (project format — normative), `BACKLOG.md` (work breakdown), PLATFORM-NOTES.md (gotchas & verification recipes).
...
- The landing mechanism is verified: `product.json` `"homepage"` selects the landing client, and since 0.18.0 the server panics at startup when no client is registered at `/clients/<product.homepage>` (PLATFORM-NOTES #25) [VERIFIED — rig re-baselines at pkg_version 0.18.3 and 0.18.5, 2026-07-30; STATE.md D27(a)]. The rig sets `"homepage": "main"`. Remaining scaffold task: set `homepage` to the tC4 client. That closes OPEN-QUESTIONS #4.
...
| A-5 | REVISED 2026-07-06 (the project owner, via imported UI design): Check surface is design-native (design/tC4-2/translationCore.dc.html) with triage + tC3 selections (STATE.md D2); tc-checking-tool-rcl *components* are not embedded. word-aligner/usfm-js logic and BURRITO-SPEC payloads unchanged — OPEN-QUESTIONS #6/#7 close via the uw-tc4 prototype instead of an RCL mount | AGREED |
