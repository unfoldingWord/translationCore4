...
| `src/data/export/usfm.ts`, `weave.mjs` | producers `usfm-aligned` ("USFM, aligned", file `<BOOK>-aligned-<YYYY-MM-DD>.usfm`) and `usfm-plain` ("USFM, plain", file `<BOOK>-<YYYY-MM-DD>.usfm`), for Bible projects. Plain is the stored book file byte for byte. Aligned is `weaveBook(usfm, alignments)`: each verse with a valid §5.1 record (no `invalid` flag, and I-3 holds) gets `wordaligner.merge`; every other verse keeps its stored text. `weave.mjs` is the one weave: `conformance/validate.mjs` imports it. The stored project does not change. | https://github.com/unfoldingWord/translationCore4/issues/19 | none (`readBook`, `readAlignments`) | `test/export/usfm.test.ts` (the round trip on the conformance sample, with `test/helpers/zaln.ts` `extractVerseFromZalnUsfm` as the oracle) | `j07` USFM block: unweave every verse of the download; the plain download equals the stored file |
