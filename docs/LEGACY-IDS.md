...
| `Tn` / `M-n` / `E-Jn` | T21, M-5, E-J4 | A row of the internal test plan (`E-Jn` was the end-to-end row for journey `Jn`) | Maintainer workspace. The public equivalent of the aggregate is `npm run verify` |
...
| `Jn` / `@incN` | J5, @inc2 | A user journey / its test tag | `docs/JOURNEYS.md` defines every journey and names its proof (D69, 2026-09-06). Journey tests live in `e2e/` (`npm run test:e2e`) |
| `JC-n` | JC-20 | A journal conformance check group (BURRITO-SPEC §8, Appendix A) | `docs/BURRITO-SPEC.md` Appendix A; run `npm run validate:journal`. Before 2026-09-06 these were written `Jn`: a bare `Jn` inside §8, Appendix A, `journal/`, or `conformance/` text from before that date means `JC-n` (D69) |
...
| `sample-burrito/`, `sample-burrito-validation/` | harness check 23 | The conforming sample project and the executable conformance harness (34 checks + journal 59) | Published: the master is `conformance/` in this repository (issue #47, closed 2026-08-14); the legacy names decode to it |
