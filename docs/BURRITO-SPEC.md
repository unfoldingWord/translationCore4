...
- [R-8.5.6] **Book generations (causal, no timestamp fallback).** Every `align.verse.set`, `check.decision.set` and `note.add` carries `generation` = the `ts` of the `book.add` generation it belongs to; omitting it refuses the fold, seeded events included. These are the operations whose first write on a key MAY be rootless (R-8.5.15), so the stamp is their only causal anchor. A `text.verse.set` is never rootless on a live key (R-8.5.15): its generation is the root of its `base` chain, and it MAY carry `generation` as well; a stamp that is present MUST be an §8.2 ts, and a stamp that names any root other than the chain's is retained and reported (`prior-generation`), never projected [decided 2026-09-05 — D68]. After a `book.remove` + re-add, prior-generation records are retained and reported (`prior-generation`) — never silently resurrected, never deleted — and the stamp decides, never the HLC.
