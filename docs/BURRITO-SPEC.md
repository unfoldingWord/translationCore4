...
**Reference implementation:** `conformance/sample-burrito/` (a conforming project) and `conformance/` (the Phase-1 suite, 34 checks, plus the journal suite — each suite's own summary line is the authoritative count [VERIFIED 2026-08-14 at the initial flip change set: 34/34 + journal 137/137; the review rounds changed the journal total]). Both live in `conformance/` in this repository (published per issue #47, D48). If this document and the harness disagree, the harness is wrong. The harness MUST be fixed to match this document. Then this document's version bumps.
...
<project>/                              git repository
  ...
  ingredients/
    ...
    checking/
      ...
      journal/<actorId>/segments/<encoded-ts>.action.json  role x-journal — Phase 2, the ONLY stream form (§8.1)
...
The **journal conformance suite** (§8.9; J1–J30 per Appendix A — the suite's own summary line is the authoritative check count) validates the full §8 semantics, the publication topology, and the intake rejection rules separately: `npm run validate:journal`. Both suites: `npm run validate:all`.
...
- **The action-container contract.** A segment file is one JSON document:
  `{"container": 1, "body": "<string>", "sha256": "<hex>"}` — `body` is the action JSON, serialized exactly once as a string; `sha256` is computed over the UTF-8 bytes of that exact body string. The body parses to `{"events": [event, …]}`: all events of ONE store mutation. **One declarative schema (the round-5 simplification):** a single central validator defines the legal event — the §8.3 envelope, the §8.5 per-op payload (the disposition enum and its required `to`/`post` fields included), and the action shape (a non-empty array, element shape total over nulls, strictly ascending `ts`, one actor) — and it is applied identically at seal (the writer MUST refuse to seal a violation), at segment validation/intake (a violating segment is invalid), and at the fold (a violating event refuses). Writer and reader are symmetric by construction because both call the same validator; there are no per-site handcrafted checks to drift apart. **Multi-scope actions are allowed**: one segment MAY carry `_project`-scope and book-scope events together (one action, one file — this dissolves the cross-stream atomicity problem).
...
- **One stream form.** Sealed action segments are the ONLY journal stream form. The pre-ratification draft's NDJSON read-compat form (`<BOOK>.<seq>.jsonl`) was DELETED in the round-5 simplification: no released project ever contained it — it existed only in this suite's fixtures, and its exemptions kept spawning bypasses. Readers MUST NOT accept any other form; intake rejects `.jsonl` files like any non-whitelisted file (§8.7). Non-owning actors never modify another actor's files.
...
Input: the union of all actors' journal events (all sealed segments — the only stream form). Algorithm:
...
**Version:** 1.2 · 2026-08-14 · companion to BURRITO-SPEC 1.8 §8 (the D48 flip change set adds J21–J30; the round-5 simplification retires J11 with the deleted legacy stream form; version 1.1 · 2026-07-10 accompanied 1.3-draft).
...
| J11 | §8.1 | **RETIRED (round-5 simplification).** J11 tested the pre-ratification NDJSON read-compat stream form, which was deleted from the format — no released project ever contained it. The ID is not reassigned (D44) | retired |
