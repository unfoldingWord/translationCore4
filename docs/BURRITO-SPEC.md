...
- [R-8.5.20] **A renumber is one structural action.** A renumber is one `text.structure.apply`. Each moved slot's transition names the old key's live text head as its source and states that head's text unchanged. The fold then projects the moved text byte-identical at the new key and nothing at the old key. All moves of one renumber are in one action: when one source is missing or stale, no move applies (R-8.5.4).
