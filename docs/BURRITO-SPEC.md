...
**Book generations (normative).** A `book.add` `ts` is the book's **generation root** (it already confers head identity per slot). After a `book.remove`, a later `book.add` of the same code starts a NEW generation: a record whose lineage predates the current generation — its structural ancestor lies outside the current chain, or it has no structural ancestor and its `ts` does not postdate the generation root — **does not project**. It is retained and reported for review (`prior-generation`): quarantine, not resurrection, not deletion (the D36 posture). Current-generation records are untouched by the rule.
