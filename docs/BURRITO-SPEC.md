...
**Version:** 1.17 · 2026-09-25 (1.17: the re-key rules for renumbered verses (issue #209, option A approved by the owner on the issue; closes #219 per D79 point 10) — a renumber is one `text.structure.apply` (R-8.5.20); a re-keyed alignment keeps its content and projects `invalid: true`, because the project frame does not move (R-8.5.21); a decision never re-keys, so its decisionKey-targeted notes stay with it (R-8.5.22, R-8.5.12 amended, R-8.5.5 no longer needs a decision number form); collisions refuse before the action (R-8.5.23); a record keyed to one verse is on every span slot that contains it (R-8.5.24); §5.1 and §5.2 state the renumber outcome; normative gate → 96 rules; conformance suite → 50 checks, Stage-1 38; journal suite adds JC-34. 1.16: §10.6 adds the OPTIONAL `obs-tq` member, the OBS translation questions read like `translationQuestions` (D75 amendment, issue #331); normative gate → 91 rules; conformance suite → 48 checks. 1.15: §10.6 defines OBS set completeness and the OPTIONAL `obs-images` override pin, while the bundled default image pack remains independent of project pins (D75, issue #288); normative gate → 90 rules. 1.14: two project kinds — §1 states them and a new §10 "The OBS project kind" holds only the differences for Open Bible Stories (`gloss/textStories`): the template layout and seed form, the frame model (the image line splits frames; exactly one paragraph per frame; byte-strict writes), the `story:frame` locator with frame 0 the title, the §5.2 story key (`checkId|obs|story|frame|occurrence`, file `checking/<toolId>/OBS.json`), three OPTIONAL §5.3 members (`obs`, `obs-tn`, `obs-twl`), and journal vocabulary version 2 (`text.frame.set`, `text.story.ref.set`, story targets on `check.decision.set`/`note.add`; `v: 1` readers untouched) (D66, D74, issue #147); the reference OBS project is `conformance/sample-burrito-obs/` built from the vendored pankosmia `text_stories` template; conformance suite → 47 checks in four groups (Stage-1 36, Stage-2 2, Phase-2 3, OBS 6), journal suite → 349 (JC-33), normative gate now covers §8 AND §10 (89 rules). 1.13: the §5.1 verse record MAY carry `done` — the translator's "this alignment is finished", set when every word is placed or by Mark valid, removed by any later alignment edit, outranked by I-3 and `invalid` (D73, issue #271); additive-optional, `schemaVersion` stays 1; conformance suite → 41 checks, Stage-1 36. 1.12: the journal conformance checks are cited as `JC-n`, same numbers, so they no longer collide with the user journeys `Jn` of `docs/JOURNEYS.md` (D69). No rule text changes; the harness check names carry the same rename, and its rule coverage is unchanged. 1.11: R-8.5.6 names the three operations that carry the mandatory `generation` stamp — `align.verse.set`, `check.decision.set`, `note.add` — and states that a `text.verse.set` takes its generation from its `base` chain and MAY carry the stamp (D68, issue #175); the reference schema and the writer already behaved this way, the sentence did not; journal suite → JC-29f. 1.10: optional per-set help slots (§5.3 — D64, epic #104/#110): a language set MAY additionally pin `translationQuestions` (tq — `parascriptural/x-bcvquestions`) and `simplifiedText` (the language's simplified, meaning-based Bible — `scripture/textTranslation`); set completeness is UNCHANGED (tn+twl+tw+tA) and readers treat an absent optional slot as covering no book, and the §8.5 `resource.pin.set` slot grammar accepts the two new set slots (it defers to §5.3); conformance suite → 40 checks, Stage-1 35, journal suite → 339 (the suite summaries are authoritative; the 1.9 line's 35/30/336 predates the E33 versification checks). History: 1.9 2026-08-24: the sha is a pin's identity — §5.3 pin and extraScripture entries require `sha` with `version` an OPTIONAL display label, §5.2 resolution records match by (repoPath + sha) (D58, completed by D59); R-8.8.2 amended: universal seeding is content-exact with record order as byte form, and a sidecar the checkpoint projection cannot represent (an unknown top-level field, or co-present §5.2 records on one identity key) MUST refuse the seed (D56); conformance suite → 35 checks, Stage-1 30, journal suite → 336. History: 1.8 2026-08-18: §8 write format + fold invariants ratified per D55 — sealed action segments replace the NDJSON stream form (D50), structural action + book generations added (#65/D48), base:null semantics (D53), prefix-collision rule (D54), textMd5/skeletonMd5 removed (D48), sync operations status-marked [PROPOSED]; rule ids [R-…] added with coverage enforced by conformance/normative/check.mjs. History: 1.7-draft 2026-08-04: Version 1.3 corrected the transport topology; JC-19–JC-20; journal suite → 59 checks. Version 1.4 encodes STATE decisions D25–D28: `ingredients/vrs.json` and the versification frame rule (§4.3, §5.2); scope range arrays, scope-filtered derivation, and partial-book legality (§3, §4.1, §4.2); stage rules S-1/S-2 made permanent — x-roles are non-durable by design and the client re-asserts them (§5.3, §6, §7); conformance suite → 30 checks. Version 1.5 promotes `extraScripture` gateway source pins to normative (§5.3 — D10/OPEN-QUESTIONS #13); conformance suite → 31 checks, Stage-1 27. Version 1.6 lands the D17/D30 two-language-set pin schema — §5.3 `schemaVersion` 2 with `languageSets.primary/fallback` (tn+twl+tw+tA each), the (tool, book) resolution record + cross-language re-attach rule in §5.2 (OPEN-QUESTIONS #28); conformance suite → 33 checks, Stage-1 29. Version 1.7 adds the carry-over rule (§5.2 — D36, 2026-08-04): the resource is the primary key, and a decision that neither re-attach pass can place is invalidated and retained, not queued for review; conformance suite → 34 checks, Stage-1 30.)
...
A **tC4 project** is one git repository. The repository is a valid Scripture Burrito (SB) at every commit. A project is one of **two kinds** [decided 2026-09-15 — D74; D66]: a **Bible project**, flavor `scripture/textTranslation` (§1–§9), or an **OBS project** (Open Bible Stories), flavor `gloss/textStories` (§10 — it states only the differences; everything else here applies). A Bible project contains:
...
3. `type.flavorType` = `{ name: "scripture", flavor: { name: "textTranslation", usfmVersion: "3.0", … } }` (an OBS project: §10.1).
...
- **`done`** (OPTIONAL, additive — decision D73, 2026-09-12, issue #271): `true` when the translator's alignment of this verse is finished. A writer sets it when an edit leaves every target word placed and no source group without a target word (tC3's "alignment complete"), and when the translator marks the verse valid with words still in `wordBank` — a Mark valid also restamps `targetVerseMd5` to the current text and clears `invalid` (the translator has re-reviewed). A writer MUST recompute `done` on every later alignment edit of the verse, including a reflow after a text edit: `true` when the edited record is fully aligned by the rule above, otherwise removed — so a Mark valid given with words in the bank does not survive a change, while a verse that stays fully aligned stays done. Readers MUST treat an absent field as not done, and MUST NOT read `done: true` as done while I-3 fails or `invalid` is set (the hash and the flag outrank it). The field carries no `false` form. The zaln export neither carries nor needs it: `done` is a record-level fact about the translator's work, not about the words. Additive-optional, so `schemaVersion` stays 1 (§9). Harness check: `alignment: done is additive — a record with done:true exports the same zaln and the field is boolean where present`.
- **Renumbered verses** [decided 2026-09-25 — owner approval of option A in the issue #209 design comment]: when a structural action re-keys an alignment record to its text's new verse key (§8.5 R-8.5.21), the record keeps `alignments`, `wordBank` and `targetVerseMd5`, and carries `invalid: true` until the translator re-aligns it. The project frame does not move in a renumber, so a different original-language verse stands behind the new key. Harness check: `alignment: a renumber re-keys the record to its text's new verse...`.
...
- **Renumbered verses** [decided 2026-09-25 — owner approval of option A in the issue #209 design comment]: a decision's `reference` is its resource row's place in the project frame, and no structural action moves it (§8.5 R-8.5.22). When a renumber moves the verse text away, the decision is invalidated and retained on its own key (`invalidated: true`, `status: "invalid"`), and the translator re-checks it. Harness check: `decisions: a renumber never moves a decision...`.
...
- **Optional per-set help slots** [decided 2026-08-27 — D64, epic #104/#110]: a language set MAY additionally pin `translationQuestions` (tq — the language's per-book comprehension-question TSVs; sb-zip flavor `parascriptural/x-bcvquestions` [VERIFIED 2026-08-27 — `unfoldingWord/en_tq` `/sb/v89.zip` metadata; export revision `97c0a13e3b84d46d0e643ba2e8e9f1c295547a58` = the v89 tag commit]) and `simplifiedText` (the language's simplified, meaning-based Bible — `en_ust` for English, `<lang>_gst` where a gateway publishes one; flavor `scripture/textTranslation`). Both slots use the §5.3 pin grammar unchanged (sha REQUIRED, `version` an optional label, `flavor` REQUIRED, `books` optional per D41). Set **completeness is unchanged**: a set without these slots is complete (tn+twl+tw+tA — D17), so every existing schemaVersion 2 file remains conformant; this is an additive, non-breaking amendment. Readers resolve the slots per (tool, book) over the same two-rung ladder as the required slots, and MUST treat an absent optional slot as covering no book — never as an error. The English install ships both pins (§5 default #1); a gateway package includes them when the owner publishes the repos [owner ruling 2026-08-27 — issue #110].
...
- **Per-pin book coverage** [decided 2026-08-07 — D41; implemented 2026-08-24 — issue #16]: every repo pin MAY carry `books`, the uppercase book codes that exact commit contains. The field is **OPTIONAL and additive**, so `schemaVersion` stays 2 (§9).
  ...
  - **Imported coverage** [decided 2026-09-26 — D82]: a tC3 import records, on each pin that it resolves, the books whose tC3 checks were made against that commit. The decisions are the evidence, so the list is a fact about the commit. It is a partial record, and a later local read of the same `repoPath` + `sha` widens it (D61).
...
A project is conforming when `conformance/`'s checks pass against it. The suite has **50 checks in four groups** — Stage-1 38, Stage-2 2, Phase-2 3, OBS 7 (the suite's own summary line is the authoritative count):

- **Stage-1 — path-authoritative conformance (38 checks).** Schema validity; ingredient integrity; versification (`vrs.json` presence, shape, scope coverage, role); scope grammar (whole-book `[]` + range arrays, with negative controls); targetBible derivability; alignment round-trip + staleness guard; selections validity + invalidation firing; decision-shape completeness incl. the additive `status` field; derive+merge progress reconstruction; scope-filtered derivation with the in-scope progress denominator; cross-language re-attach with groupId tiebreak + irreducible ambiguity left unplaced (D17); carry-over invalidation of unplaceable decisions (D36); multi-book scope; two-language-set pin completeness (D17/D30 §5.3 shape); (tool, book) resolution records + the two-rung coverage ladder; extraScripture source pins (§5.3 entry shape, unique ids, `sha` grammar); optional tq + simplifiedText set slots (§5.3 1.10 pin shape when present, absent-slot legality); zaln export; verse-span key semantics; the renumber outcome for §5.1 and §5.2 records. These checks pass on today's pankosmia-web unmodified.
...
- **OBS — the OBS project kind (6 checks, §10; D74).** Run against `sample-burrito-obs/`: schema validity with flavor `gloss/textStories`; layout equality with the vendored `text_stories` template (seed form or a §10 draft of it); the frame model; the byte-strict frame write; `currentScope` equality with the template; the version 2 fold projected onto the seed story byte for byte. Each check also fires on a deliberately broken copy. The Bible groups are unchanged by this group.

The **journal conformance suite** (§8.9; JC-1–JC-34 per Appendix A — the suite’s own summary line is the authoritative check count) validates the full §8 semantics, the publication topology, and the intake rejection rules separately: `npm run validate:journal`. Both suites: `npm run validate:all`.
...
{"v": 1, "op": "text.verse.set", "actor": "maria-x1",
 ...
 "generation": "<ts of the book.add generation this record belongs to>",  // REQUIRED on align.verse.set, check.decision.set, note.add; OPTIONAL on text.verse.set (R-8.5.6, D68)
...
|---|---|---|---|
| `text.verse.set` | (book, chapter, verseKey) | `{book, chapter, verse, text}` | LWW register with fork detection; its generation is its `base` chain's root; MAY carry `generation` (R-8.5.6, D68) |
...
| `note.add` | — (grow-only) | `{target, text}`; `target` is exactly one of `{book, chapter, verse}` or `{decisionKey}` (the **toolId-prefixed** §5.2 decision key), or — `v: 2` only — `{story, frame}` (§10.7) | grow-only; notes are permanent in `v: 1` — no delete/edit op; they project into fold output only; carries `generation` |
...
| `text.frame.set` (`v: 2`, §10.7) | (story, frame) | `{story, frame, text}`; frame `0` is the title | LWW register with fork detection; no structural chain, no generation (R-10.7.2) |
| `text.story.ref.set` (`v: 2`, §10.7) | story | `{story, text}` | LWW register with fork detection; no structural chain, no generation (R-10.7.2) |

[R-8.5.1] There are no other ops in `v: 1`; new ops bump `v` (§9). Journal `v: 1` vocabulary is textTranslation-shaped; other flavors extend by version bump, and `v: 1` writers are unaffected. `v: 2` (§10.7, D74) adds exactly the two story ops above and lets `check.decision.set` and `note.add` carry a story target; `v: 2` is a superset of `v: 1`, and a `v: 1` segment set folds unchanged.
...
- [R-8.5.4] **`text.structure.apply` is all-or-nothing.** The action carries the new skeleton, the old-key → new-key `transitions`, and `dispositions` for every affected live record — alignments, decisions, verse-targeted notes, and verse TEXT on mapped keys (a decisionKey-targeted note is never affected — R-8.5.22). A missing source, stale source head, or incomplete disposition set makes the whole event `incomplete`/`conflicted`: the pre-operation state projects unchanged, and the event is reported. [R-8.5.18] **Transitions map EVERY destination.** `transitions` maps every destination slot key of the new skeleton to `{text, sources}`: its required final text (merge/split content is stated, never inferred) and `sources` = immutable references to every affected live source head. A transition key missing from the new skeleton, a destination slot without a transition or stated text, or one source head claimed by two transitions is malformed and refuses the fold. [R-8.5.16] **Dispositions are CONSTRAINED to the computed affected set:** a disposition referencing a record outside that set MUST refuse the whole event (all-or-nothing) — without this bound, a structural action mapping one verse could carry a disposition that consumes any unrelated live record in the book. [R-8.5.5] Disposition forms are `re-key`, `replace` (the `post` MUST be a complete, schema-valid record of its surface — §5.1 for alignments, §5.2 for decisions — whose identity is consistent with the disposition's target), `invalidate-retain`, and `orphan-review`; a disposition whose `action` or `surface` is outside those enums refuses the whole event. A `re-key` destination is a §8.4 SLOT KEY, not a number: a destination like `1:02` stays `1:02`, so the record keeps naming the slot that exists. A disposition MUST NOT change a record's `generation` — conservative handling is never a re-stamp, so a quarantined record can never resurrect through one. [R-8.5.12] **A re-key destination is a slot key of the new skeleton**, for each surface that re-keys (an alignment, a verse-targeted note); the schema refuses any other destination, at seal and at fold. A `decisionKey`-targeted note never re-keys: its decision keeps its key (R-8.5.22), so the note stays with it, and a disposition that names it is outside the affected set (R-8.5.16). [R-8.5.17] **`replace` is NOT a note disposition** (notes are grow-only in `v: 1`; replacement contradicts their model), and **`re-key` and `replace` are not text-surface actions** — the transitions already state where content goes, so a live text head's dispositions are `invalidate-retain` or `orphan-review` only.
- [R-8.5.6] **Book generations (causal, no timestamp fallback).** Every `align.verse.set`, `check.decision.set` and `note.add` carries `generation` = the `ts` of the `book.add` generation it belongs to; omitting it refuses the fold, seeded events included. For the two keyed operations the first write on a key MAY be rootless (R-8.5.15), and `note.add` is grow-only with no structural ancestry at all, so for all three the stamp is the only causal anchor. A `text.verse.set` is never rootless on a live key (R-8.5.15): its generation is the root of its `base` chain, and the stamp never stands in for that chain — an edit whose `base` is unknown stays unanchored until the base arrives, stamp or no stamp. It MAY carry `generation` as well; a stamp that is present MUST be an §8.2 ts, and a stamp that does not resolve, through the converged-root alias (R-8.5.3), to the chain's generation root — another generation's root, or no `book.add` at all — is retained and reported (`prior-generation`), never projected [decided 2026-09-05 — D68]. After a `book.remove` + re-add, prior-generation records are retained and reported (`prior-generation`) — never silently resurrected, never deleted — and the causal generation decides — a verse edit's base-chain root, or the stamp where it is mandatory or present — never the HLC.
...
- **Renumbered verses** [decided 2026-09-25 — owner approval of option A in the issue #209 design comment]. A renumber changes which draft text sits at which verse key. The project versification frame does not change (R-8.5.19, §5.2 frame rule), so after a renumber a different source verse stands behind the moved text. The rules below follow from that fact.
- [R-8.5.20] **A renumber is one structural action.** A renumber is one `text.structure.apply`. A writer that renumbers MUST give each moved slot a transition that names the old key's live text head as its source and states that head's text unchanged; the fold then projects the moved text byte-identical at the new key and nothing at the old key. The fold cannot tell a renumber from a structural edit that states new text (R-8.5.18), so it does not refuse changed text: I-3 then marks a moved alignment stale as well. All moves of one renumber are in one action: when one source is missing or stale, no move applies (R-8.5.4).
- [R-8.5.21] **`re-key` keeps the content and asks for re-review.** A re-keyed record keeps every payload field and its `generation` (R-8.5.5); the structural action consumes the old head, so nothing is deleted. The fold MUST project a re-keyed alignment with `invalid: true`: its target words and `targetVerseMd5` still match the moved text, but its source words belong to the old key's source verse. The fold sets the flag, so no writer can omit it.
- [R-8.5.22] **A decision never re-keys.** A decision's `reference` names its resource row's place in the project frame (§5.2), and no structural action moves the frame. A `re-key` disposition on the decision surface refuses the whole event, at seal and at fold. The conservative form is `invalidate-retain`, which keeps the key. A `decisionKey`-targeted note therefore stays with its decision (R-8.5.12).
- [R-8.5.23] **Collisions refuse before the action.** Two cases are collisions: (a) a transition destination that is a slot of the base skeleton, whose content no transition moves elsewhere and whose own transition does not name it as a source, while it holds a live text head; (b) an alignment `re-key` onto a key that holds a live alignment the same action does not move away (a `replace` keeps its record on its key, so it does not move it away), or two alignment re-keys onto one key. The fold reports the event `conflicted` with a `collision:` detail, and the pre-action state projects unchanged. A writer SHOULD refuse a colliding renumber before it seals, and name the verse.
- [R-8.5.24] **Span membership.** For the affected set of any structural action, a record keyed to one verse `c:v` is on its own key and on every slot `c:a-b` with `a ≤ v ≤ b`. This applies to alignments, decisions and verse-targeted notes; it closes #219 (D79 point 10). A decision on a broken span stays on its key with the conservative disposition. The fold's affected set and the §8.8 reconcile builder read ONE shared predicate, so the event reconcile emits is one the fold accepts.
...
## 10. The OBS project kind (Open Bible Stories — NORMATIVE per D74, 2026-09-15)

A tC4 project is one of two kinds: a **Bible project** (`scripture/textTranslation`, §1–§9) or an **OBS project** (`gloss/textStories`, this section) [decided 2026-09-15 — D74; D66]. An OBS project is a different flavor of Scripture Burrito, and it is still a Scripture Burrito. This section states only the differences. Everything this section does not name applies unchanged: the §3 metadata rules (except rules 3 and 4, replaced by §10.1 and §10.2), the §5.3 pins, the §5.4 settings, the §6 server rules, and the §8.1–§8.3 journal envelope. The reference project is `conformance/sample-burrito-obs/`; the checks are the `OBS` group of `npm run validate` and the `JC-33` group of `npm run validate:journal` (§7, Appendix A). Vocabulary: `CONTEXT.md` ("story", "frame", "story title", "reference line", "frame locator", "image pack").

### 10.1 Kind and flavor

- [R-10.1.1] An OBS project's `type.flavorType` is `{ "name": "gloss", "flavor": { "name": "textStories" }, "currentScope": … }`. The core Scripture Burrito schema allows nothing else in the flavor object (`sb-schema/gloss/text_stories.schema.json`, `additionalProperties: false`). `metadata.json` MUST validate against the bundled schema. §3 rules 1, 2, 5 and 6 apply; the ingredient `mimeType` of a story file is `text/markdown`.

### 10.2 Repository layout

The layout is pankosmia's `text_stories` content template, byte for byte (`conformance/fixtures/text_stories/`, vendored from `pankosmia/resource-core` at the rig's pin; provenance in its README).

```
<project>/
  metadata.json                         SB metadata (§3 + §10.1)
  .gitignore                            MUST contain: **/*.bak
  ingredients/
    LICENSE.md                          copied from the source
    content/01.md … content/50.md       one story file per story (§10.3)
    content/front/title.md              copied from the gateway source; not drafted in Phase 1
    content/front/intro.md              copied from the gateway source; not drafted in Phase 1
    content/back/intro.md               copied from the gateway source; not drafted in Phase 1
    checking/
      translationNotes/OBS.json         role x-check-decisions  (§10.5)
      translationWords/OBS.json         role x-check-decisions  (§10.5)
      resources.json                    role x-resource-links   (§5.3 + §10.6)
      settings.json                     role x-check-settings   (§5.4)
      journal/<actorId>/…               role x-journal          (§8.1, §10.7)
```

- [R-10.2.1] The ingredient set of an OBS project is the template's ingredient set plus the `checking/` sidecars: `content/01.md` to `content/50.md`, `content/front/title.md`, `content/front/intro.md`, `content/back/intro.md` and `LICENSE.md`. There is no `vrs.json` and no alignment sidecar. The front, back and LICENSE files are copied from the gateway source and MUST be byte-identical to it. (The core Scripture Burrito example uses `content/front.md` and `content/back.md`; `unfoldingWord/en_obs` and the pankosmia template use the `front/` and `back/` directories. tC4 follows `en_obs` and pankosmia.)
- [R-10.2.2] The ingredient path of story `N` is `content/NN.md`, with a two-digit story number (`content/01.md`). Story numbers are 1 to 50. The scope is always all fifty stories.
- [R-10.2.3] `type.flavorType.currentScope` is the template's table, copied verbatim. It is keyed by the Bible passages the stories retell (the schema requires the book-code grammar of §3 rule 4). A writer MUST NOT reconstruct, reorder or filter it; the harness checks equality with the template.
- [R-10.2.4] **The seed form.** A new OBS project is the template's fifty story files in the seed form: each title line reduced to `# N.` with no title text; each frame the image line followed by an empty paragraph, in the template's exact byte form (three blank lines between image lines, five after the last); no reference line. The image lines keep the source's cdn URL text. Pictures resolve at render time (a project ingredient with role `x-obsimages`, then a pinned `x-obsimages` burrito, then the default image pack) and the image line is never rewritten.

### 10.3 The frame model

A story file is one title line, then frames, then an optional reference line, all separated by single blank lines. Line ends are LF.
...
- [R-10.3.1] **The image line splits frames.** The title line is line 1, `# N.` or `# N. Title`. Every line of the form `![…](…)` starts a frame. A frame is its image line and the one paragraph that follows it. Text before the first image line, other than the title line, is malformed.
- [R-10.3.2] **Exactly one paragraph per frame.** A frame's paragraph is the contiguous non-blank lines after its image line. Single newlines inside the paragraph are kept. A second paragraph in one frame is malformed. A writer MUST remove blank lines inside a frame before it seals (D74 §5) and MUST refuse frame text that still contains a blank line, an image line, a line in the reference-line form, or a line in the title form.
- [R-10.3.3] **The reference line** is the last non-blank line of the file when it has the form `_…_` and is not an image line: `_A Bible story from: Genesis 1-2_`. It is drafted. It has no frame locator and is never a check target. A story with a reference line and no frame is malformed. The reference line's text MUST be non-empty; no operation removes it.
- [R-10.3.4] **Writes are byte-strict outside their region (the D8 analogue).** The region of frame `F` is every line after its image line up to the next image line, the reference line, or the end of the file. The region of the title is line 1. The region of the reference line is that line when present, else the trailing blank lines after the last frame's paragraph. A write replaces its region and nothing else: the title, every image line, every other frame and the reference line are byte-identical afterwards. A frame written to the empty text returns its region to the seed form (R-10.2.4). A write to a frame the story does not have is refused: the frame set is fixed by the source (R-10.7.5).

### 10.4 The frame locator

- [R-10.4.1] A frame is addressed as `story:frame` without leading zeros (`1:1`), as OBS Translation Notes writes it. Frame `0` is the story title (`1:0`). The reference line has no locator. In JSON, `story` and `frame` are integers.

### 10.5 Check decisions

- [R-10.5.1] A story check decision is a §5.2 record whose `contextId.reference` is `{ "story": 1, "frame": 1 }` instead of `{bookId, chapter, verse}`; the two forms MUST NOT mix in one reference. Its identity key is the §5.2 five-part form with the literal `obs` in the book position: `checkId|obs|story|frame|occurrence`, so decision keys, note targets and dispositions keep one grammar. The sidecar is `checking/<toolId>/OBS.json` with `"book": "OBS"`; `toolId` is `translationNotes` or `translationWords`; the `resource` resolution record (§5.2, D30) is required for `(toolId, OBS)`. The check rules of §5.2 carry over: invalidate-and-retain when the frame text changes (D36), comments, bookmarks, `status`. There is no alignment layer and no `align.verse.set` for stories. `occurrence` is `1` in every OBS help row and is not counted [VERIFIED — `unfoldingWord/en_obs-twl` master, 2,393 rows, 2026-09-15; D74 §8].

### 10.6 Resources

- [R-10.6.1] A §5.3 language set MAY additionally pin three OBS text members: `obs` (the gateway-language stories, the source text, flavor `gloss/textStories`), `obs-tn` (flavor `peripheral/x-obsnotes`) and `obs-twl` (flavor `parascriptural/x-bcvarticles`) [flavor labels corrected 2026-09-16 — DCS sb-zip export metadata at the pinned tags; see `docs/PLATFORM-NOTES.md` §35]. Translation Words and Translation Academy are shared with Bible projects. Each member uses the §5.3 pin grammar unchanged (`sha` REQUIRED, `version` an optional label, `flavor` REQUIRED). A complete Bible set remains `translationNotes` + `translationWordsLinks` + `translationWords` + `translationAcademy`. A complete OBS set is `obs` + `obs-tn` + `obs-twl` + `translationWords` + `translationAcademy`; the two Bible-only members are not required. A reader treats an absent member as covering nothing. The OBS members project after `simplifiedText`, in the order `obs`, `obs-tn`, `obs-twl`.
- [R-10.6.2] A language set MAY pin `obs-images` (flavor `peripheral/x-obsimages`) as an image-pack override. It projects after `obs-twl`. Its absence does not affect either completeness rule. At render time an image resolves by its Markdown URL basename, first hit wins: a project ingredient whose role is `x-obsimages`, the primary set's `obs-images` pin, the fallback set's `obs-images` pin, then the bundled sha-pinned default `uW/obs_images_360`. A missing or unavailable candidate falls through. Resolution MUST NOT rewrite the Markdown image line. The default pack is installer data, not an implicit project pin.
- [R-10.6.3] A language set MAY pin `obs-tq` (the OBS translation questions, flavor `peripheral/x-obsquestions`, one `OBS.tsv` in the tQ column layout with `story:frame` references) [decided 2026-09-18 — D75 amendment, issue #331; flavor from the DCS sb-zip export metadata at tag v10]. It is OPTIONAL in the sense of `translationQuestions` (D64): a reader treats its absence as covering nothing, and it counts toward neither completeness rule. It projects after `obs-twl` and before `obs-images`. The §5.3 pin grammar applies unchanged.

### 10.7 Journal vocabulary version 2

- [R-10.7.1] **Envelope version 2.** The story vocabulary is `v: 2`. `v: 2` is a superset of `v: 1`: every `v: 1` op validates and folds unchanged under either version, and a `v: 1` segment set folds exactly as before (§8, R-8.5.1). The two story ops and the story targets below REQUIRE `v: 2`; a `v: 1` reader never meets them. `v: 3` is unknown (R-8.3.9).
- [R-10.7.2] **Two ops.** `text.frame.set {story, frame, text}` sets one frame's paragraph; frame `0` sets the title text (one line, trimmed). `text.story.ref.set {story, text}` sets the reference line's text without the underscores. Each is an LWW register with fork detection (§8.6) keyed `frame|story|frame` and `ref|story`. Stories have no structural chain and no generation: a rootless first write on a frame is ordinary (like a pin), same-actor writes advance linearly (R-8.3.3), two actors' rootless writes on one frame fork and are surfaced, identical twins auto-merge and are accounted (R-8.6.4), a `supersedes` resolves (§8.3), and no frame head is ever retained as `rootless-base` or `prior-generation`. Payload identity is `{story, frame, text}` and `{story, text}`.
- [R-10.7.3] **Story targets.** `check.decision.set` MAY carry a decision whose reference is `{story, frame}` (§10.5), and `note.add` MAY carry the target `{story, frame}` or the decision key of a story decision. Such an event MUST NOT carry `generation` (stories have no book generations; R-8.5.6 binds book targets only) and MUST be `v: 2`. The fold anchors it by nothing: it resolves like a pin, never as absent-book or prior-generation.
- [R-10.7.4] **Projection and checkpoint.** The fold projects each story as its folded frame texts (frame `0` the title) and its reference line — `stories[story] = {frames: {frame: text}, ref}`. At checkpoint (§8.7) the story file is a DERIVED shared file: the folded state spliced onto the BASE story bytes — the committed file, or the seed form (R-10.2.4) — through the §10.3 writers, in ascending frame order, then the reference line. The base story is a MANDATORY checkpoint input like `metadata.json` (R-8.7.4): a folded story with no base refuses the checkpoint. Story files are enumerated for divergence like every derived file (R-8.7.5): an out-of-band edit or deletion is detected, never silently repaired. An OBS project has no book, so it needs no versification frame at checkpoint.
- [R-10.7.5] **No structural operations for stories.** The frame set is fixed by the source. No op adds, removes, splits or merges a frame; `book.add`, `text.skeleton.set` and `text.structure.apply` do not apply to stories. A write to a frame the base story does not have (frame 17 of a 16-frame story) is refused at the writer and at the checkpoint. Reconcile of an out-of-band story edit (§8.8) is not defined in this version: such an edit is divergence (R-10.7.4) and the app MUST stop, not overwrite.

## Appendix A — Journal conformance test plan (§8 and §10 companion)

**Version:** 2.3 · 2026-09-25 (2.3: JC-34, renumbered verses — the re-key rules R-8.5.20–R-8.5.24, issue #209) · 2.2 · 2026-09-15 (2.2: JC-33, the OBS story kind — §10's rule ids are claimed like §8's, and the gate scans both sections — D74) · 2.1 · 2026-09-06 (2.1: check groups are cited as `JC-n` — D69) · 2.0 · 2026-08-18 · companion to BURRITO-SPEC 1.8 §8. (2.0 replaces the 1.x hand-maintained coverage table: coverage of §8's `[R-…]` rules is now MACHINE-CHECKED — each rule id is claimed inside a live check's name as `[covers R-…]`, and `node conformance/normative/check.mjs` fails on any uncovered rule, stale claim, or duplicate id. The gate tracks ids, not text: a rewording keeps its claim and is visible in the spec diff; per-statement mutation hardening remains open (D55 residuals). This appendix is an index, not a second source of truth.)
...
JC-1 HLC ordering/ratchet/overflow · JC-2 skeleton codec byte-identity · JC-3 fold determinism (property) · JC-4 LWW linear history · JC-5 fork detection + identical-payload auto-merge · JC-6 supersedes resolution · JC-7 I-3 staleness composition · JC-8 out-of-band reconcile · JC-9 three-device convergence · JC-10 sneakernet ≡ merge · JC-11 RETIRED (tested the deleted NDJSON stream form; the id is not reassigned — D44) · JC-12 end-to-end two-actor git regeneration · JC-13 de-duplication by ts · JC-14 schema totality + version policy · JC-15 Phase-1 sidecar seed migration · JC-16 section-vs-verse machinery · JC-17 definitions closure · JC-18 Pankosmia transport scratch behavior · JC-19 actor-publication lifecycle · JC-20 zero-trust intake rejection · JC-21 the structural action (all-or-nothing, dispositions) · JC-22 structural lineage + pending propagation · JC-23 sealed action segments (container, filenames, immutability, containment) · JC-24 staged-intent republication · JC-25 vrs first-value register · JC-26 pin golden projection · JC-27 removal semantics per surface · JC-28 actor binding + same-actor rule · JC-29 book generations (causal quarantine) · JC-30 checkpoint regeneration set + divergence · JC-31 value grammars, two independent layers · JC-32 base semantics, conservation + exclusivity (property) · JC-33 the OBS story kind (§10): frame registers, story targets + the §5.2 story key, the §10.6 pin slots, checkpoint story inputs, version policy (the file-level OBS checks are the `OBS` group of `validate.mjs`) · JC-34 renumbered verses: one action, re-key with re-review, collisions, span membership, old data over every record kind + checkpoint mirror

### Acceptance

- `npm run validate:journal` exits 0 — the suite's own summary line is the authoritative count — **and** the Phase 1 suite still passes (`npm run validate`, incl. the `OBS` group), **and** `npm run validate:all` runs both, **and** `node conformance/normative/check.mjs` exits 0 (every `[R-…]` rule of §8 and §10 claimed by a live check).
