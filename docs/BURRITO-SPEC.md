...
The **journal conformance suite** (§8.9; JC-1–JC-32 per Appendix A — the suite’s own summary line is the authoritative check count) validates the full §8 semantics, the publication topology, and the intake rejection rules separately: `npm run validate:journal`. Both suites: `npm run validate:all`.
