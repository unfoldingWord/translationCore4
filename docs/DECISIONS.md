...
tC4's separate pair. **Note (2026-09-08, D71):** the byte-traceability rationale applies where an
sb-zip export exists. The two lexicons have none, so D71 pins them in the `uW` org by commit sha.
...
[publication note: recorded 2026-09-03 from the owner's architecture notes of
2026-08-29, which are held outside this repository and are not cited as evidence. This
...
nothing else names the generation. A `text.verse.set` MUST NOT be rootless on a live key
(R-8.5.15), so its `base` chain reaches the `book.add` of its generation, and the fold
...
## D71 (2026-09-08, project-owner ruling) **The lexicons are the `uW`-org burritos, pinned by commit sha and fetched as commit archives. Hardcoded until the December 2026 Greek New Testament brings its own lexicon.** [issue #218]

Context [VERIFIED 2026-09-08 — DCS API, downloads, `pankosmia/uw-client-checks` source]:
`git.door43.org/uW/en_ugl` (master `d9d29e2d589258ce27f92b59f753a3af03ab7a72`) and
`git.door43.org/uW/en_uhl` (master `72df5ac25acf9d51e826b20e3ad883a5a657ef4e`) are Scripture
Burritos with flavor `peripheral/x-lexicon`, generated by Burrito Truck on 2025-11-24. Their
content is `ingredients/content/<strongs>.json` with `{brief, long}`: tC3's Strong's JSON,
re-wrapped. The platform's own checking client hardcodes these two repos. The
`unfoldingWord/en_ugl` v0.5 repo is 2017 markdown that nothing reads; `unfoldingWord/en_uhl` is
not a burrito. Neither org's lexicon repo has an sb-zip export (`/sb/` answers 500 or 404).
No ULT artifact declares a lexicon (`manifest.yaml` `relation`, the sb-zip `metadata.json`,
the `uW/en_ult` burrito): the dependency is a product decision, and every consumer hardcodes it.

Ruling:
1. `installedSuite.js` `resources.lexicon` pins `uW/en_ugl` and `uW/en_uhl` at the shas above,
   sha-only (no tag exists; the version label is never invented).
2. A sha-only pin whose commit no tag names is fetched as the Gitea commit archive
   `/archive/<sha>.zip`. Gitea records the packed commit in the zip's archive comment; the
   fetch verifies that comment against the pin (D23b: the expected sha comes from the pin, not
   the archive). This path lives in the app's fetch code and is shared by the build's resource
   cache. Never a branch: the request names the commit.
3. The desktop artifact bundles both under `_local_/_sideloaded_/uw--<repo>` beside the eight
   `unfoldingWord` repos (D70 item 5: "all eleven pinned resources" reads as the ten repos —
   en_tw serves two slots, D34). `BUILD-MANIFEST.json` records `"version": null` for them.
4. Known limits, accepted: no permanent home for the lexicon; hardcoded pins are hard to update;
   the uW burritos carry no DCS identity in their metadata, so the installed-resource scan
   (`identifyOne`, `src/data/installed.ts`) cannot identify them until a consumer needs it. A new
   Greek New Testament with its own lexicon is expected in December 2026; the lexicon path is
   reworked against that release in early 2027. This ruling holds until then.

## D72 (2026-09-11, project-owner rulings) **Increment 6 is defined: three journeys, one epic, the date holds. Suggestions learn from the project's own alignments and are never saved unconfirmed.** [grill-with-docs session 2026-09-11; epic #261; issues #1, #9, #40, #50, #94, #134, #213, #255–#262]
