// Journal conformance suite — BURRITO-SPEC §8 / Appendix A (JC-1–JC-32), spec 1.8 (the D48
...
// ---------- JC-1: HLC ----------
{
  ...
  check('JC-1: same-ms issues increment counter; advance resets [covers R-8.2.2]', t1 < t2 && t2 < t3 && parseTs(t2).counter === 1 && parseTs(t3).counter === 0);
  const far = '2030-01-01T00:00:00.000Z|00ff|actor-b';
  c.ratchet(far);
  const t4 = c.issue();
  check('JC-1: receive-ratchet — next issue sorts after everything seen [covers R-8.2.4]', t4 > far, t4);
  const o = makeClock('actor-a', () => 2000);
  let last = ''; for (let i = 0; i <= 0x10000; i++) last = o.issue();
  check('JC-1: counter overflow bumps physical ms and resets counter [covers R-8.2.3]', parseTs(last).physical === 2001 && parseTs(last).counter === 0);
  prop('JC-1: property — issue order ≡ string sort order across interleaved clocks [covers R-8.2.1]',
...
// ---------- JC-2: skeleton codec ----------
{
  check('JC-2: SLOT is U+0001 (encoding sanity)', SLOT.length === 1 && SLOT.charCodeAt(0) === 1);
  const fixtures = {
  ...
  for (const [name, f] of Object.entries(fixtures)) {
    ...
    check(`JC-2: byte-identical recompose [covers R-8.4.2] — ${name}`, recompose(skeleton, verses) === f, `${Object.keys(verses).length} verses`);
  }
  let rejected = false; try { decompose('\\id X\n\\v 1 bad' + SLOT); } catch { rejected = true; }
  check('JC-2: U+0001 in source is rejected [covers R-8.4.3]', rejected);
  const base = fixtures['structure-rich'];
  prop('JC-2: property — mutate any verse, recompose still exact [covers R-8.4.2]',
...
// ---------- JC-3: fold determinism (property) ----------
{
  ...
  prop('JC-3: property — fold(events) ≡ fold(shuffle) ≡ fold(partition union) ≡ fold(+duplicates) [covers R-8.6.1]', genEvents, (events) => {
...
// ---------- JC-4–JC-7, JC-13, JC-14 unit tests over one scenario ----------
{
  ...
  check('JC-4: LWW linear — later event with base=head replaces [covers R-8.3.1]', lin.books.TIT.verses['1:1'] === 'uno v2\n' && lin.forks.length === 0);
  ...
  check('JC-5: fork detected — same base, different actors+payloads; provisional = max ts, surfaced [covers R-8.3.2 R-8.6.4]',
  ...
  check('JC-5: identical-content fork auto-merges (distinct events by identity, no review item) [covers R-8.2.5 R-8.6.4]', twin.forks.length === 0 && twin.books.TIT.verses['1:1'] === 'versión A\n');
  ...
  check('JC-6: supersedes both heads resolves the fork [covers R-8.3.5]', resolved.forks.length === 0 && resolved.books.TIT.verses['1:1'] === 'resuelta\n');
  ...
  check('JC-6: a plain continuing edit advances its branch but does NOT resolve the fork',
  ...
  check('JC-7: I-3 composition — valid → text edit invalidates → re-align revalidates [covers R-8.5.10]',
  ...
  check('JC-13: duplicate identical events are a no-op [covers R-8.2.5]', deepEq(fold([...seedEvts, forkA, forkA, forkA]), fold([...seedEvts, forkA])));
  let dupThrew = false; try { fold([...seedEvts, forkA, { ...forkA, text: 'otro\n' }]); } catch { dupThrew = true; }
  check('JC-13: same ts + different content refuses (corrupt union) [covers R-8.2.5]', dupThrew);
  // v: 2 is the §10 story vocabulary (spec 1.14); the unknown version is 3
  let vThrew = ''; try { fold([mkEvent({ op: 'text.verse.set', actor: 'x-actor', ts: t(0, 0, 'x-actor'), v: 3 })]); } catch (e) { vThrew = e.message; }
  let opThrew = ''; try { fold([mkEvent({ op: 'text.verse.merge', actor: 'x-actor', ts: t(0, 0, 'x-actor') })]); } catch (e) { opThrew = e.message; }
  check('JC-14: unknown v / unknown op refuse with clear messages [covers R-8.3.9 R-8.4.5 R-8.5.1]', vThrew.includes('version') && opThrew.includes('unrecognized op'), `"${vThrew.slice(0, 40)}" / "${opThrew.slice(0, 40)}"`);
}

// ---------- JC-14 (vocabulary closure): the op set is CLOSED — the schema's known-op set
//   IS the §8.5 table, member for member, and no op names a section (§8.4a) ----------
{
  const SPEC_OPS = [
    ...
    'text.frame.set', 'text.story.ref.set', // §10 (D74), `v: 2`
  ];
  check('JC-14: the schema\'s known-op set EQUALS the §8.5 table — 12 `v: 1` ops + the 2 `v: 2` story ops (§10), set-equal, and no op matches /section/i (no section op in the vocabulary, §8.4a) [covers R-8.4.5 R-8.5.1]',
...
// ---------- JC-14b (round 6): the schema is TOTAL over the envelope — every field the fold
//   dereferences has a shape rule; malformed input gets a clean rejection, never a crash ----------
{
  ...
  check('JC-14b: every wrong-typed envelope/ts-shaped field is refused CLEANLY at seal AND at fold — no raw TypeError, no silent pass',
    allClean, details.join(' · '));
}

// ---------- JC-14c (round 7): the schema is TOTAL over PER-OP PAYLOADS too — one shared
//   §5.1/§5.2 record validator; every payload field the fold dereferences has a rule ----------
{
  ...
  check('JC-14c: the reviewer\'s event — a decision with an EMPTY contextId — is refused AT SEAL (and cleanly at fold), never sealed-then-crashed',
  ...
  check('JC-14c: every wrong-typed PER-OP payload field the fold dereferences is refused CLEANLY at seal AND at fold — no raw TypeError, no silent pass',
    allClean, details.join(' · '));
}

// ---------- JC-14d (round 7 internal pre-pass): the §5.2 identity-key STRING grammar —
...
//   serializer/validator pair, everywhere an identity-key string appears ----------
{
  ...
  check('JC-14d: every malformed §5.2 identity-key STRING is refused CLEANLY at seal AND at fold — one five-part grammar, shared with the serializer',
    allClean, details.join(' · '));
  ...
  check('JC-14d: a pipe-less decisionKey never enters the fold — pre-fix it silently bypassed the generation quarantine (projected past a re-add); post-fix it is refused, never mis-filed',
...
// ---------- JC-14e (round 7 internal pre-pass): supersedes cannot self-reference ----------
{
  ...
  try { sealAction([ev]); } catch (e) { sealMsg = e.message; }
  try { fold([ev]); } catch (e) { foldMsg = e.message; }
  check('JC-14e: an event listing its OWN ts in supersedes is malformed — refused at seal AND at fold (dangling supersedes refs stay harmless-by-construction: they filter no live head) [covers R-8.3.5]',
    sealMsg !== '' && foldMsg !== '', `seal="${sealMsg.slice(0, 50)}" fold="${foldMsg.slice(0, 50)}"`);
}

// ---------- JC-14f (round 9): INVARIANT I-4 has an implementation. §8.5 said "writers MUST
...
//   identity-bearing values and object KEYS are REFUSED. ----------
{
  ...
  check('JC-14f (I-4): sealAction NORMALIZES every text a writer journals — verse content, skeletons, note text, initial verse content, structural destination text, and settings/metadata string values are NFC in the sealed bytes [covers R-8.5.9]',
  ...
  check('JC-14f (I-4): an NFD identity component is REFUSED at seal AND at fold — pre-fix it produced a SILENT IDENTITY SPLIT (two records, identity keys that print identically, no fork, no retained entry) [covers R-8.5.13]',
  ...
  check('JC-14f (I-4): the NFC form of the same identity still seals and folds — the refusal adds no false rejection',
  ...
  check('JC-14f (I-4): the refusal is CLASS-level — quoteString, decisionKey, dotted paths, the vrs frame name, and every object KEY at every depth refuse a non-NFC value; only CONTENT is transformed [covers R-8.5.13]',
  ...
  check('JC-14f (I-4): what the writer seals is exactly what its own reader validates — the normalized action round-trips through validateSegment carrying the NFC text',
    reread.ok === true && reread.events[0].text === NFC && reread.events[0].text !== NFD,
    `re-read text is NFC: ${reread.ok && reread.events[0].text === NFC}`);
}

// ---------- JC-14g (round 9): the schema is TOTAL over HOSTILE input too — a verdict is
...
//   only, and a `base` that actually precedes its own event (§8.1/§8.3). ----------
{
  ...
  check('JC-14g: a hostile deeply-nested segment — a small fraction of the 4 MiB cap — gets a VERDICT, not a RangeError: validateSegment returns {ok:false} so onInvalid fires and the rest of the stream still reads',
  ...
  // and the honest neighbour in the same directory still reads
  {
    ...
    check('JC-14g: one hostile segment no longer bricks the actor stream — it is reported and skipped, and the honest segments beside it still read',
  ...
  check('JC-14g: every hostile-but-schema-shaped payload is refused CLEANLY at seal AND at fold — depth bound, path-segment bound, plain-JSON object kinds only, no own `__proto__` key, and a `base` that precedes its own ts',
    allClean, details.length ? `missed: ${details.join(' · ')}` : `${rows.length} firing cases`);
  check('JC-14g: the legitimate shapes are untouched — a 3-segment path, a 4-deep value and an ordinary earlier base all still seal and fold',
    (() => {
...
// ---------- JC-8: out-of-band reconcile ----------
{
  ...
  check('JC-8: reconcile emits seeded supersede; fold equals the edited file [covers R-8.8.1]', recEvents.length === 1 && recEvents[0].seed.source === 'out-of-band-usfm' && after.books.TIT.usfm === edited && after.forks.length === 0);
...
// ---------- JC-9 + JC-10: convergence & sneakernet via real sealed segments ----------
{
  ...
  check('JC-9: three-device disjoint edits converge — identical bytes, zero forks [covers R-8.1.16]',
  ...
  check('JC-10: sneakernet (file copy union) ≡ in-memory union ≡ cross-device read', deepEq(foldA, foldI) && deepEq(foldI, foldM));

  fs.rmSync(tmp, { recursive: true, force: true });
}

// ---------- JC-12: end-to-end git — sealed segments + fold + derived-file regeneration ----------
{
  ...
  check('JC-12: naive merge of two checkpoints conflicts on the derived USFM', conflicted && /^UU ingredients\/TIT.usfm/m.test(status), status.trim().split('\n')[0]);
  ...
  check('JC-12: resolve-either-side + regenerate → two-parent commit; both edits present; bytes = fold(union)',
  ...
  check('JC-12: both actors\' journals present after integration', deepEq(journals, ['device-aa', 'device-bb', 'seed-actor']));
  fs.rmSync(T, { recursive: true, force: true });
}

// ---------- JC-15: Phase-1 sidecar seed migration (golden vs the real sample) ----------
{
  const { events, decisionFiles, alignmentFiles, books } = buildSeed();
  const out = fold(events);
  check('JC-15: seeded fold reproduces the committed USFM byte-exactly [covers R-8.8.2]', out.books.TIT.usfm === books.TIT && out.books.JON.usfm === books.JON);
  ...
  check('JC-15: decisions round-trip exactly (records deep-equal, counts match)',
  ...
  check('JC-15: alignment records round-trip exactly (incl. targetVerseMd5) [covers R-8.8.2]',
  ...
  check('JC-15: migrated alignments are valid against the folded text (I-3 carries over) [covers R-8.8.2]', out.invalid.length === 0, `${out.invalid.length} invalid`);
}

// ---------- JC-15 (full state): fold(seed) == state, byte-for-byte, on a PARTIAL-SCOPE fixture ----------
...
try {
  ...
  check('JC-15 (full state): fold(seed) reproduces EVERY derived file of the partial-scope fixture byte-for-byte (scope, pins, settings, metadata, full §5.1 alignment fields — INVALIDATED records included) [covers R-8.8.2]',
    mismatches.length === 0 && extras.length === 0,
    JSON.stringify({ mismatches, extras }));
  check('JC-15 (full state): the seeded scope is the fixture\'s actual partial scope, not a hardcoded whole-book default',
    deepEq(out.scope, { TIT: ['1:1-1:3'] }), JSON.stringify(out.scope));
  check('JC-15 (full state): the seeded alignment record carries all §5.1 fields (sourceVersion, invalid) through the fold',
  ...
  check('JC-15 (full state): the invalidated records seed and fold correctly — the stale alignment is reported by I-3, the invalidated decision is retained',
...
// ---------- JC-16: drafting by section vs checking by verse (\ts\* = presentation only; target text never carries it — §4.1/§8.4a). Fixtures model IMPORTED files + section-save batching. ----------
{
  const F = '\\id TIT test\n\\c 1\n\\ts\\*\n\\p\n\\v 1 Pablo siervo de Dios,\n\\v 2 con esperanza de vida eterna,\n\\ts\\*\n\\p\n\\v 3 a su debido tiempo,\n\\v 4 a Tito, verdadero hijo.\n\\ts\\*\n\\p\n\\v 5 Por esta causa te dejé en Creta,\n';
  const { skeleton, verses } = decompose(F);
  check('JC-16: imported \\ts\\* round-trips — chapter-opening in skeleton; boundaries in preceding verse content [covers R-8.4.1]',
  ...
  check('JC-16: structure-only edit (stripping an imported \\ts\\*) does NOT invalidate the verse\'s alignment (I-3 on plain text) [covers R-8.5.10]',
    afterMove.invalid.length === 0 && !afterMove.books.TIT.verses['1:2'].includes('\\ts\\*'),
    `invalid=${afterMove.invalid.length}`);
  check('JC-16: a word edit on the same verse DOES invalidate', afterWords.invalid.length === 1);
  ...
  check('JC-16: section save is per-verse events sharing a batch; only the double-edited verse forks [covers R-8.3.7 R-8.4.6]',
    merged.forks.length === 1 && !!fork && merged.books.TIT.verses['1:4'].includes('REDRAFTED'),
    `forks=${merged.forks.map((f) => f.key).join(',')}`);
  check('JC-16: fork heads carry the batch id — review queue can group per-verse forks by section action',
    forkBatches.includes(batchTs), JSON.stringify(forkBatches));
}

// ---------- JC-17: remaining-definitions closure — settings.set, meta reserved roots, orphaned alignments, NFC (I-4) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-17: settings.set — LWW per path; §5.4 state projects from the fold (no shared mutable settings.json)',
  ...
  check('JC-17: project.meta.set targeting a reserved root (ingredients/format/type/meta) refuses with a clear message',
  ...
  check('JC-17: project.meta.set on an allowed path still folds', okMeta.projectMeta['identification.name.en'] === 'Renamed');
  ...
  check('JC-17: alignment on a removed verse slot is orphaned → invalid[] regardless of matching hash [covers R-8.6.6]',
  ...
  check('JC-17: I-4 motivation — NFC and NFD forms of identical text produce different md5 (why writers MUST normalize)',
  ...
  check('JC-17: note.add — both target shapes accumulate grow-only (no LWW, no deletion)',
...
// ---------- JC-18: marries Pankosmia's transport to the journal (sealed segments as the
//   on-disk artifact — the only stream form). Mirrors their PullFromDownloaded choreography. ----------
{
  ...
  check('JC-18: Pankosmia transport over disjoint journals — scratch merge is CLEAN (their conflict-abort never fires) [covers R-8.1.16]',
    jr.conflict === false && unmerged === '');
  check('JC-18: converged — both translators\' concurrent same-book edits present, zero forks',
  ...
  check('JC-18: SAME transport on a shared whole-file same-line edit DOES conflict — the case their model aborts on, and the journal removes',
    wr.conflict === true);
  fs.rmSync(tmp, { recursive: true, force: true });
}

// ---------- JC-19: repeat publication without receiving main — sealed-segment publications. ----------
{
  ...
  check('JC-19: actor publication commits add only their owned sealed segments (existing segments untouched)',
  ...
  check('JC-19: A1 then B1 integrate through disposable scratch and regenerate cleanly',
  ...
  check('JC-19: merging the full offline working checkpoint would conflict after main regenerated',
  ...
  check('JC-19: A2 re-submits without receiving B1 — integration is clean and A2+B1 survive',
  ...
  check('JC-19: receive validates a rebuilt replacement before swap; old working repo is untouched and replacement has the union',
...
// ---------- JC-20: intake is zero-trust even when git merges cleanly. Scratch validation rejects
...
//   bytes, modification of accepted sealed segments, and invalid incoming segments (§8.1). ----------
{
  ...
  check('JC-20: intake rejects a clean git merge that changes any shared/non-journal path',
  ...
  check('JC-20: intake rejects truncation/rewrite of accepted segments and foreign-actor edits; accepted main remains byte-identical [covers R-8.1.15]',
  ...
  check('JC-20: intake rejects modification of an accepted sealed segment and any invalid incoming segment (§8.1 asymmetric rule, incoming side) [covers R-8.1.5]',
  ...
  check('JC-20: whitelist-only intake — an arbitrary new file under the actor directory is rejected',
    miscErrors.some((e) => e.includes('notes.txt')) && miscErrors.some((e) => e.includes('segments/README.md')),
    JSON.stringify(miscErrors));
  check('JC-20: whitelist-only intake — a JSONL stream is rejected like any other non-whitelisted file (one stream form: sealed segments) [covers R-8.1.10]',
    miscErrors.some((e) => e.startsWith('not-whitelisted:') && e.includes('JON.00001.jsonl')), JSON.stringify(miscErrors));
  check('JC-20: whitelist-only intake — a malformed actor.json is rejected (shape validated, actorId must match the directory) [covers R-8.1.13]',
    miscErrors.some((e) => e.includes('actor.json')), JSON.stringify(miscErrors));
  // the `segment-misnamed` branch was DEAD CODE until round 9: every JC-20 case fed either
  ...
  check('JC-20: whitelist-only intake — a VALID segment whose filename is not its own first event ts is rejected (§8.1); the misnamed branch now has a firing case',
  ...
  check('JC-20: whitelist-only intake — a valid new sealed segment and a well-formed actor.json are explicitly allowed',
  ...
  check('JC-20: intake rejects DELETION of an accepted segment — journals are permanent history, no compaction in v:1 [covers R-8.1.15]',
...
// ---------- JC-21: text.structure.apply — all-or-nothing structural actions (#65, D48) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-21: span create (9,10 → 9-10) — one atomic event; merged text stated, never inferred',
  ...
  check('JC-21: span break (9-10 → 9,10) — the split text is stated per destination slot',
  ...
  check('JC-21: a missing source reference reports incomplete; the pre-operation state projects unchanged (no stubs, no partial projection) [covers R-8.5.4]',
  ...
  check('JC-21: a stale source head (concurrent verse edit) reports conflicted; the pre-operation state (with the edit) projects unchanged [covers R-8.5.4]',
  ...
  check('JC-21: malformed structural events refuse the fold (transition outside the skeleton; a slot without a transition; one source claimed twice) [covers R-8.5.18]',
  ...
  check('JC-21: fold determinism under permutation holds with structural events [covers R-8.6.1]',
  ...
  check('JC-21: a structural event that omits a disposition for a live verse-targeted note on a mapped key is refused as incomplete; pre-op state projects [covers R-8.5.4]',
  ...
  check('JC-21: a structural event that omits a disposition for a live decision on a mapped key is refused as incomplete; pre-op state projects [covers R-8.5.4]',
  ...
  check('JC-21: duplicate/conflicting dispositions for one record refuse the fold (malformed event)',
    dupDisp.includes('disposition'), `"${dupDisp.slice(0, 60)}"`);
}

// ---------- JC-21c (review round 2): invalidated records and decisionKey-targeted notes are affected records too ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-21c: omitting a disposition for an INVALIDATED alignment on a mapped key refuses the event (invalidated records are retained state, D36) [covers R-8.5.4]',
  ...
  check('JC-21c: omitting a disposition for an INVALIDATED decision on a mapped key refuses the event [covers R-8.5.4]',
  ...
  check('JC-21c: a re-keyed invalidated alignment survives under the new key with invalid: true preserved',
  ...
  check('JC-21c: an invalidate-retain decision keeps its identity — its decisionKey-targeted note needs no disposition and still projects',
...
// ---------- JC-21b: reconcile emits COMPLETE conservative dispositions (§8.8 + #65 v2) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  const after = fold([...events, ...recEvents]);
  check('JC-21b: reconcile emits dispositions for the alignment, the decision, AND the verse-targeted note on a removed key — the structural event applies (complete) [covers R-8.8.1]',
    !!structEv &&
  ...
  check('JC-21b: reconcile dispositions are conservative — invalidate-retain/orphan-review, never a guessed re-key [covers R-8.8.1]',
  ...
  check('JC-21b: after the conservative reconcile the decision is retained invalidated, never deleted (D36)',
    (after.decisions.translationWords || []).length === 1 && after.decisions.translationWords[0].invalidated === true);
}

// ---------- JC-21d (review round 4): dispositions are CONSTRAINED to the affected set (§8.5) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-21d: a disposition referencing a record OUTSIDE the computed affected set is refused whole (all-or-nothing) — structural actions cannot consume unrelated records [covers R-8.5.16]',
  ...
  check('JC-21d: the unrelated 1:1 alignment still projects (the rogue event never applied)',
  ...
  check('JC-21d: a rogue note disposition (unmapped verse) is refused the same way; the note survives [covers R-8.5.16]',
    refusedN.includes('affected') && (outN === null),
    refusedN ? `"${refusedN.slice(0, 60)}"` : `note projected=${outN?.notes.length}`);
}

// ---------- JC-21e (review round 5): the disposition SCHEMA binds — enum + required fields (§8.5) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  for (const [label, d] of cases) {
    ...
    check(`JC-21e: disposition schema violation is refused whole [covers R-8.5.5] — ${label}`,
  ...
  check('JC-21e: a schema-valid disposition still applies (re-key to a mapping target)',
  ...
  for (const [label, d] of [
    ...
    check(`JC-21e: ${label} is refused whole — re-key/replace are not text actions; a live text head's dispositions are invalidate-retain/orphan-review only [covers R-8.5.17]`,
      refused.includes('disposition'), `"${refused.slice(0, 90)}"`);
  }
}

// ---------- JC-21f (round 6): replace.post is a VALIDATED post-state (§5.1/§5.2 shapes,
//   identity-consistent with the disposition's target); note replacement is rejected ----------
{
  ...
  for (const [label, dispositions] of cases) {
    ...
    check(`JC-21f: ${label} is refused whole [covers R-8.5.5]`,
  ...
  check('JC-21f: decision replace whose post identity mismatches the §5.2 target is refused whole',
  ...
  check('JC-21f: note replacement is REJECTED — notes are grow-only in v1 (replace contradicts their model) [covers R-8.5.17]',
  ...
  check('JC-21f: a complete §5.1 post-state with consistent identity still applies',
    !!good.alignments.TIT?.['1:2'] && good.pendingStructural.length === 0,
    JSON.stringify(good.alignments.TIT));
}

// ---------- JC-22: structural lineage — branch-local effects, retention, sequential chains (#65 ruling) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-22: concurrent structural actions fork on the skeleton key — both heads live, review-queue material',
    forked.forks.some((f) => f.key === 'skel|TIT') && forked.pendingStructural.length === 0,
    JSON.stringify(forked.forks.map((f) => f.key)));
  check('JC-22: fork effects are branch-local — the winner projects its own move only; the losing branch\'s move never leaks [covers R-8.6.8]',
  ...
  check('JC-22: the losing branch\'s post-images are retained for review, not silently dropped',
  ...
  check('JC-22: edits on both sides of a structural fork — each descendant projects only under its own ancestor',
    bothSides.books.TIT.verses['1:4'] === 'cuatro B\n' && !('1:3' in bothSides.books.TIT.verses),
    JSON.stringify(bothSides.books.TIT.verses));
  check('JC-22: the losing branch\'s descendants remain retained for review (excluded by ancestry, not guesswork) [covers R-8.6.8]',
  ...
  check('JC-22: sequential structure changes chain as ordinary head lineage (the second bases on the first; descendants follow)',
  ...
  check('JC-22: note retention across a renumber — the dispositioned note re-keys to the new verse; the other note survives unchanged',
  ...
  check('JC-22: slot-changing text.skeleton.set refuses (use text.structure.apply); a slot-preserving header edit still folds [covers R-8.5.14]',
  ...
  check('JC-22: text.skeleton.set with a NULL base refuses to ACT — retained and reported (`rootless-structural`, D53d), the book.add skeleton still projects, and the fold never throws',
  ...
  check('JC-22: an UNKNOWN base is pending until it arrives — the pre-event state projects, nothing is guessed',
  ...
  check('JC-22: when the missing base arrives, the pending link folds — determinism is per event-SET',
...
// ---------- JC-22b (review round 4): a stale KNOWN base is no escape from the topology rule (§8.4) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  const t = (s, c, a) => `2026-08-02T00:00:${String(s).padStart(2, '0')}.000Z|000${c}|${a}`;
  const skelA = `\\id TIT\n\\c 1\n\\p\n\\v 1 ${SLOT}1:1${SLOT}\\v 2 ${SLOT}1:2${SLOT}`;
  ...
  check('JC-22b: a stale KNOWN-base text.skeleton.set that reverses a structural change is REFUSED — the comparison binds to the CURRENT projected skeleton, never the historical base',
  ...
  check('JC-22b: a slot-preserving skeleton edit against the current topology still folds (the rule blocks topology reversal, not header edits)',
  ...
  check('JC-22b: a same-actor skeleton edit based past the actor\'s own later head is refused — no branch can silently lose the structural post-images',
  ...
  check('JC-22b: byte-invariant — an accepted (chain-linked) skeleton.set leaves every verse projection byte-identical',
  ...
  check('JC-22b: a cross-actor same-base skeleton edit FORKS explicitly (surfaced, provisional, branch-coherent) — never silently applied, never silently dropped',
...
// ---------- JC-22c (round 6): pending propagates TRANSITIVELY — a descendant of a pending
//   skeleton link is pending, not accepted (§8.4/§8.5) ----------
{
  ...
  check('JC-22c: a skeleton edit based on a PENDING link pends too — it never becomes a head, and the projection is unchanged (verse bytes intact)',
  ...
  check('JC-22c: pending propagates through a three-deep chain — every descendant pends [covers R-8.5.2]',
  ...
  check('JC-22c: when the missing ancestor arrives, the whole chain folds and verse bytes are untouched (byte invariant holds)',
...
// ---------- JC-22d (round 7): ONE accepted-structural-predecessors set — pending
...
//   text.structure.apply are one chain-link class, §8.4/§8.5) ----------
{
  ...
  check('JC-22d: a skeleton edit based on a PENDING text.structure.apply pends too — pending crosses op types; the pre-op slots and bytes project unchanged',
    out.pendingStructural.length === 2 &&
  ...
  check('JC-22d: a text.structure.apply based on a PENDING skeleton link pends too — the inverse direction holds (one accepted-predecessor rule, both op types)',
  ...
  check('JC-22d: pending propagates through a MIXED three-deep chain (skeleton.set → structure.apply → skeleton.set) — all three pend [covers R-8.5.2]',
  ...
  check('JC-22d: when the missing ancestor arrives, the whole mixed chain folds — verse bytes untouched across the op-type boundary',
    resolved.pendingStructural.length === 0 && resolved.books.TIT.usfm.includes('via tail') &&
    resolved.books.TIT.verses['1:1'] === 'uno\n' && resolved.books.TIT.verses['1:3'] === 'dos\n',
    JSON.stringify({ pending: resolved.pendingStructural, header: resolved.books.TIT.usfm.split('\n')[0], verses: resolved.books.TIT.verses }));
}

// ---------- JC-23: sealed action segments — the §8.1 container contract ----------
{
  ...
  check('JC-23: a valid segment round-trips; filenames are ts-encoded (: → _, | → ,) and sort in ts order [covers R-8.1.2]',
  ...
  check('JC-23: a torn segment is unpublished as a whole — no partial action ever folds [covers R-8.1.6]',
  ...
  check('JC-23: a checksum-failing segment is invisible as a whole (parse/checksum validity IS the commit marker) [covers R-8.1.1 R-8.1.6]',
  ...
  check('JC-23: the 4 MiB segment limit binds the writer, and an oversize file is invalid to readers [covers R-8.1.9]',
  ...
  check('JC-23: multi-scope actions in ONE segment (book + settings + metadata) fold correctly [covers R-8.1.3]',
  ...
  check('JC-23: the WRITER refuses to publish another actor\'s events into this actor\'s directory (§8.1/§8.3 actor binding, writer side — the rule both intakes already applied) [covers R-8.1.12]',
  ...
  check('JC-23: a segment whose events name another actor than its directory is refused (actor binding, §8.3) [covers R-8.1.12]',
    invalids3.length === 1 && String(invalids3[0]).startsWith('actor-mismatch'), JSON.stringify(invalids3));
  fs.rmSync(tmp, { recursive: true, force: true });
}

// ---------- JC-23b: accepted segments are IMMUTABLE at the writer, and invalidity is never silently dropped (§8.1) ----------
{
  ...
  check('JC-23b: writeSegment branches — byte-identical rewrite accepts idempotently; a different valid action at the same path is REJECTED, bytes untouched [covers R-8.1.4 R-8.1.5]',
  ...
  check('JC-23b: an invalid existing segment is recovered ONLY through verified staged-intent republication — a plain write refuses; republication over a VALID segment refuses [covers R-8.1.8]',
  ...
  check('JC-23b: readSegments/readUnion NEVER silently drop an invalid segment — the default surfaces it (throws); an explicit handler collects and reads the valid remainder [covers R-8.1.7]',
...
// ---------- JC-23c (review round 3): segment filenames are legal on Windows AND under §2 (§8.1) ----------
{
  ...
  check('JC-23c: segment filenames carry NO Windows-reserved character, no reserved device name, no leading dot, no trailing dot/space',
    names.every((n) => !WINDOWS_RESERVED.test(n) && !WINDOWS_DEVICE.test(n) && !n.startsWith('.') && !/[. ]$/.test(n)),
    JSON.stringify(names[0]));
  check('JC-23c: segment filenames satisfy the §2 ingredient-path constraints too',
    names.every((n) => !ING_FORBIDDEN.test(n)),
    JSON.stringify(names[0]));
  const segmentTs = filesAll.segmentTs;
  check('JC-23c: the encoding is INJECTIVE and reversible — every risky ts character round-trips through the filename [covers R-8.1.2]',
  ...
  check('JC-23c: filename sort equals ts sort within an actor directory (fixed-width escape positions) [covers R-8.1.2]',
  ...
  check('JC-23c: a ts round-trips write→read — events return in ts order and each file is named by its encoded ts',
    got.length === tss.length && got.every((e, i) => e.ts === tss[i]) && deepEq(onDisk, names),
    JSON.stringify(onDisk));
  fs.rmSync(tmp, { recursive: true, force: true });
}

// ---------- JC-23d (review round 4): the container contract binds the ACTION shape too (§8.1) ----------
// §8.1 MUST audit, one firing case each: outer parse + container:1 + body-string + sha256
// (JC-23 torn/checksum), 4 MiB (JC-23), directory actor binding (JC-23/readSegments), reserved
// reader field (JC-29e), and — added here — non-empty events, strict ts order, one actor.
{
  ...
  check('JC-23d: an EMPTY events array is invalid — an action is one store mutation, at least one event (no later crash on events[0]) [covers R-8.1.1]',
    rEmpty.ok === false, JSON.stringify(rEmpty));
  const rOrder = validateSegment(craft([ev(2, 'actor-a'), ev(1, 'actor-a')]));
  check('JC-23d: a mis-ordered events array is invalid — the contract requires ts order inside the action [covers R-8.1.1]',
  ...
  check('JC-23d: a duplicated ts inside one action is invalid (strictly ascending — one actor cannot issue the same ts twice) [covers R-8.1.1]',
    rDup.ok === false, JSON.stringify(rDup.ok === false ? rDup : '(validated ok)'));
  const rMixed = validateSegment(craft([ev(1, 'actor-a'), ev(2, 'actor-b')]));
  check('JC-23d: events naming more than one actor in one segment are invalid (one action, one actor, §8.1) [covers R-8.1.1]',
  ...
  check('JC-23d: sealAction refuses to seal an empty or mis-ordered action (writer side of the same contract)',
  ...
  check('JC-23d: a well-formed multi-event action still validates', rGood.ok === true && rGood.events.length === 2);
}

// ---------- JC-23e (round 6): no filesystem traversal via a ts-shaped field; writer touches
//   the filesystem LAST (§8.1/§8.2) ----------
{
  ...
  check('JC-23e: a traversal-shaped ts is refused by the SCHEMA at seal — the exact §8.2 grammar binds, actor-slug charset included',
  ...
  check('JC-23e: writeActionSegment refuses the traversal and writes NOTHING outside the actor segments directory',
  ...
  check('JC-23e: the path derivation REFUSES containment escape independently of the schema (defense in depth)',
  ...
  check('JC-23e: writeActionSegment([]) is a clean schema refusal and touches the filesystem NOT AT ALL (no directory created)',
...
// ---------- JC-23f (round 9): the §8.1 containment guarantee is a FILESYSTEM guarantee.
...
//   reader trusted absolutely. ----------
{
  ...
  // unfoldable from one stray file. Both intakes already checked this; the reader now does.
  {
    ...
    check('JC-23f: a MISNAMED segment is invisible — the filename IS the first event\'s ts (§8.1). Pre-fix a stray file published a second body at the same ts and made the project permanently unfoldable',
  ...
  // (2) a DANGLING SYMLINK at a segment path
  {
    ...
    check('JC-23f (SECURITY): a DANGLING SYMLINK at a segment path no longer reads as "path free" — the writer REFUSES and creates nothing outside the project (pre-fix it wrote through the link)',
    ...
    check('JC-23f (SECURITY): the READER refuses the same link rather than following it — containment is enforced on both sides of the file',
  ...
  // (3) a SYMLINKED `segments` directory relocates the whole stream
  {
    ...
    check('JC-23f (SECURITY): a SYMLINKED `segments` directory relocates the entire stream — writer and reader both refuse it, and nothing is written outside',
  ...
  // (4) the actor directory is a NAME, resolved through ONE constructor
  {
    ...
    check('JC-23f (SECURITY): a traversal-shaped actor directory is REFUSED — pre-fix `readSegments(journal + "actor-a/../actor-d")` returned actor-d\'s stream while the caller believed it held actor-a\'s (basename saw only the normalized path)',
    ...
    check('JC-23f: actorDirFor is the ONE actor-directory constructor — it applies the §8.1 slug grammar and refuses anything that escapes the journal root [covers R-8.1.11]',
  ...
  // (5) the cap applies BEFORE the read (stat guard), and segmentName is injective by grammar
  {
    ...
    const seen = [];
    const got = readSegments(dir, (f, r) => seen.push(r));
    check('JC-23f: an oversize segment is reported from its STAT — the 4 MiB cap applies before the read, not after it (reading first costs ~3x the file in RSS to reach the same verdict) [covers R-8.1.9]',
    ...
    check('JC-23f: the filename encoding is injective BY GRAMMAR — segmentName refuses a non-ts outright, so `a:b` and `a_b` can never encode to one name (layer-2 independence, §8.1)',
  ...
  // (6) the §2 ipath grammar covers the characters that are not printable at all
  {
    ...
    check('JC-23f: the §2 ipath grammar refuses control characters, NUL, bidi overrides, Windows reserved device names and trailing dots — a §2 list of PUNCTUATION said nothing about names that stop meaning what they look like',
      accepted.length === 0, accepted.length ? `still accepted: ${accepted.map(([l]) => l).join(', ')}` : `${gaps.length} firing cases`);
    check('JC-23f: the legitimate §2 paths are untouched by the hardening',
...
// ---------- JC-24: staged-intent (outbox) republication — exact bytes (§8.1 asymmetric rule, local side) ----------
{
  ...
  check('JC-24: a torn local segment publishes nothing; republishing the staged bytes yields a byte-identical segment and the full action [covers R-8.1.8]',
...
// ---------- JC-25: project.vrs.set — the immutable first-value register (§8.5) + byte-exact projection (§8.7) ----------
{
  const vrsBytes = fs.readFileSync(ING('vrs.json'), 'utf8');
  ...
  check('JC-25: the first value binds; an identical repeat de-duplicates (no conflict surfaced)',
    first.vrs.bytes === vrsBytes && deduped.vrs.bytes === vrsBytes && deduped.vrsRejected.length === 0);
  check('JC-25: ANY different second value is surfaced and never applied — regardless of actor or base (not LWW)',
    rejected.vrs.name === 'eng' && rejected.vrs.bytes === vrsBytes && deepEq(rejected.vrsRejected, [v2.ts]),
    JSON.stringify(rejected.vrsRejected));
  check('JC-25: projection reproduces ingredients/vrs.json byte-exactly from the stored raw bytes (§8.7 derived list includes vrs.json)',
  ...
  check('JC-25: the creation seed segment carries project.vrs.set with seed.source "creation"',
    seeded[0].op === 'project.vrs.set' && seeded[0].seed.source === 'creation' && seededOut.vrs.bytes === vrsBytes);
}

// ---------- JC-26: the pin golden projection — events → byte-equivalent §5.3 resources.json ----------
{
  const sample = JSON.parse(fs.readFileSync(ING('checking/resources.json'), 'utf8'));
  ...
  check('JC-26: the sample exercises the 1.10 optional slots — fallback pins both, primary omits both (D64)',
  ...
  check('JC-26: pin events project to a byte-equivalent §5.3 resources.json (the golden projection, from the real sample\'s pins)',
    projected === expected, projected === expected ? `${projected.length} bytes` : 'byte mismatch');
  const rng = mulberry32(SEED + 26);
  check('JC-26: the pin projection is deterministic under event permutation',
  ...
  check('JC-26: pin removal ({slot, removed: true}) projects to absence',
  ...
  check('JC-26: an out-of-grammar pin slot refuses the fold (§5.3 slot grammar is the merge identity)',
    badSlot.includes('not a §5.3 slot'), `"${badSlot.slice(0, 60)}"`);
}

// ---------- JC-27: removal semantics per surface (§8.5 — JSON null is not absence) ----------
{
  ...
  check('JC-27: settings/meta unset ({path, removed: true}) folds to absence [covers R-8.5.11]',
    !('ui.pane' in withRm.settings) && !('identification.abbreviation.en' in withRm.projectMeta));
  check('JC-27: the projected settings document after a removal is byte-equal to one where the path was never set',
  ...
  check('JC-27: settings/meta {path, value: null} STORES null — the path is PRESENT with value null, distinct from never-set and from removed: true [covers R-8.5.11]',
  ...
  check('JC-27: alignment removal is the explicit empty-state payload — the record projects (empty), it does not vanish [covers R-8.5.11]',
  ...
  check('JC-27: decisions are never deleted — the invalidated record is retained in full, and no removal op exists in the vocabulary [covers R-8.5.11]',
...
// ---------- JC-28: actor binding + the same-actor linear rule (§8.3) ----------
{
  ...
  check('JC-28: the fold refuses an event whose actor differs from its ts actor (actor binding) [covers R-8.1.12]',
  ...
  check('JC-28: same-actor events with a STALE base advance linearly — an actor never forks against itself (§8.3:334) [covers R-8.3.3]',
  ...
  check('JC-28: a MISSING base on a live key is a writer DEFECT, not a linear advance — the write is retained (`rootless-base`) and the actor\'s real head still projects (§8.5, round 9) [covers R-8.5.15]',
  ...
  check('JC-28: the same stale base from a different actor still forks (the rule is same-actor only)',
    crossActor.forks.length === 1, JSON.stringify(crossActor.forks));
}

// ---------- JC-29: self-contained book.add — multi-key head identity + scope reconstruction (§8.5/§8.7) ----------
{
  ...
  check('JC-29: book.add is self-contained — one event creates the slot topology; an uncovered slot projects the ___ stub [covers R-8.6.7]',
    out.books.TIT.verses['1:1'] === 'uno\n' && out.books.TIT.verses['1:2'] === '___\n' && out.books.JON.verses['1:1'] === '___\n');
  check('JC-29: every produced head carries the book.add ts (the multi-key head-identity rule)',
  ...
  check('JC-29: a subsequent event using the book.add ts as base advances linearly (no fork)',
    advanced.forks.length === 0 && advanced.books.TIT.verses['1:2'] === 'dos B\n');
  check('JC-29: a checkpoint reconstructs type.flavorType.currentScope from folded scope state (§3 rule 4 — [] and range arrays)',
  ...
  check('JC-29: book.add without skeleton or scope refuses (self-contained is mandatory)',
    noSkel.includes('skeleton') && noScope.includes('scope'));
}

// ---------- JC-29c (review round 3): generations are CAUSAL — the generation field beats the HLC (§8.5) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-29c: an offline later-ts EDIT of a prior-generation record never projects against gen-2 — generation mismatch quarantines REGARDLESS of ts [covers R-8.5.6]',
    (out.decisions.translationWords || []).length === 0 && !out.alignments.TIT?.['1:1'],
    JSON.stringify({ decs: out.decisions, align: out.alignments.TIT }));
  check('JC-29c: the later-ts prior-generation edit lands in retained[] as prior-generation (quarantine, not resurrection, not deletion)',
  ...
  check('JC-29c: a record stamped with the current generation root projects, and the generation field never leaks into the projected §5.1 record [covers R-8.6.7]',
  ...
  check('JC-29c: universal seeding stamps generation from the seed\'s own book.add — every seeded record is current-generation',
...
// ---------- JC-29d (review round 3): decisionKey-targeted notes quarantine with their book's generation (§8.5) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-29d: a prior-generation decisionKey-targeted note never projects — the generation filter reaches notes with no target.book [covers R-8.5.6]',
    !out.notes.some((n) => n.ts === noteOld.ts) && !out.notes.some((n) => n.ts === noteLate.ts),
    JSON.stringify(out.notes.map((n) => n.text)));
  check('JC-29d: both notes are QUARANTINED as prior-generation — the parsed §5.2 bookId finds the root; the stamp beats the later ts [covers R-8.5.6]',
  ...
  check('JC-29d: a current-generation decisionKey-targeted note projects (the quarantine binds to the generation, not to the target shape)',
    cur.notes.some((n) => n.ts === noteCur.ts),
    JSON.stringify(cur.notes.map((n) => n.text)));
}

// ---------- JC-29e (review round 4): omitting `generation` is REFUSED, never a quarantine bypass (§8.5) ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-29e: a v1 align/decision/note event with NO generation stamp is REFUSED as malformed — omission is not a quarantine bypass [covers R-8.5.6]',
  ...
  check('JC-29e: the refusal covers all three stamped ops (align.verse.set, check.decision.set, note.add) [covers R-8.5.6]',
  ...
  check('JC-29e: a seed-flagged event without a generation stamp is REFUSED — seed is not a self-declared bypass (the seeder always stamps, §8.8) [covers R-8.3.8 R-8.5.6]',
    seedRefused.includes('generation'),
    seedRefused ? `"${seedRefused.slice(0, 70)}"` : `projected: ${JSON.stringify(seedOut?.decisions)}`);
}

// ---------- JC-29f (D68, issue #175): the stamp is OPTIONAL on text.verse.set — the base chain is its generation ----------
{
  const E = (op, actor, ts, base, extra) => mkEvent({ op, actor, ts, base, ...extra });
  ...
  check('JC-29f: a text.verse.set WITHOUT a generation stamp folds and projects — its base chain is its generation (R-8.5.15), the stamp is not mandatory here [covers R-8.5.6]',
  ...
  check('JC-29f: a text.verse.set whose stamp names the chain\'s own root projects — the optional stamp agrees with the chain [covers R-8.5.6]',
  ...
  check('JC-29f: a text.verse.set whose stamp names a root OTHER than its chain\'s is retained as prior-generation and its text never projects [covers R-8.5.6]',
  ...
  check('JC-29f: a text.verse.set with an UNKNOWN base and a current-root stamp does not project — the stamp never stands in for the base chain [covers R-8.5.6 R-8.5.15]',
  ...
  check('JC-29f: a text.verse.set generation stamp that is not an §8.2 ts is REFUSED as malformed [covers R-8.5.6]',
    refused.includes('generation'),
    refused ? `"${refused.slice(0, 80)}"` : 'accepted');
}

// ---------- JC-30: unjournaled-ingredient tolerance + whole-surface divergence detection (§8.5/§8.8) ----------
{
  const { events, decisionFiles } = buildSeed();
  // R-8.7.2's overlay half needs DIRECT assertions here (review of PR #85: removing the
  // overlay from checkpoint.mjs left this block green; only unrelated JC-15/JC-31 checks
  ...
  check('JC-30: the checkpoint regeneration set is EXHAUSTIVE per §8.7 — USFM, alignment + decision sidecars, resources, settings, vrs, metadata; no unjournaled class appears [covers R-8.7.1 R-8.7.3]',
  ...
  check('JC-30: the projected metadata.json reconstructs type.flavorType.currentScope from folded scope state AND applies the project.meta.set overlay — a folded set appears, a folded {removed:true} path is DELETED from the document (§8.7) [covers R-8.7.2]',
    (() => {
  ...
  check('JC-30: divergence detection covers every derived shared file — an out-of-band sidecar edit is detected, never silently overwritten [covers R-8.7.5]',
  ...
  check('JC-30: a DELETED derived file is divergence — expected-from-fold files are enumerated, not only present-on-disk ones [covers R-8.7.5]',
    ...
    JSON.stringify(cls.diverged));
  check('JC-30: ingredients/audio/ files are tolerated — never divergence, never regenerated or deleted at checkpoint [covers R-8.7.3]',
    cls.tolerated.includes('audio/JON-1.mp3') && !cls.diverged.includes('audio/JON-1.mp3'));
}

// ---------- JC-30b (review round 4): an exhaustive checkpoint REQUIRES its mandatory inputs (§8.7) ----------
{
  const { events, decisionFiles } = buildSeed();
  const out = fold(events);
  const baseMetadata = JSON.parse(fs.readFileSync(path.join(BURRITO, 'metadata.json'), 'utf8'));
  ...
  check('JC-30b: derivedProjections without baseMetadata THROWS — metadata.json is mandatory checkpoint state, never silently omitted [covers R-8.7.4]',
  ...
  check('JC-30b: emitting a §5.2 decision file without its (tool, book) resolution record THROWS — `resource` is required derive-time state (D30) [covers R-8.7.4]',
  ...
  check('JC-30b: with complete inputs, every emitted decision file carries its §5.2 resource record and metadata.json is present',
...
// ---------- JC-31 (round 8): ONE value-grammar module. Every constrained primitive that
...
//   the schema (layer 1) AND, independently, by its consumer (layer 2). ----------
{
  ...
  // --- the canonical book set does not drift from the product's own table ---
  {
    ...
    if (!fs.existsSync(src)) {
      check('JC-31: SKIP — canonical book-code drift guard needs the product tree (../src/data/bookNames.ts)', true, 'prerequisite absent');
    } else {
      ...
      check('JC-31: the grammar\'s §2 canonical book set equals the product\'s BOOK_NAMES keys, in canon order (no drift between harness and client)',
  ...
  // --- FINDING 1: the structural base rule, one rule for every non-root structural op ---
  {
    ...
    check('JC-31 finding 1: text.structure.apply on an UNKNOWN base PENDS (incomplete) and never applies — the pre-operation slot set projects unchanged',
    ...
    check('JC-31 finding 1: a base naming a text.verse.set is REFUSED — a structural op may only chain to a structural predecessor (§8.5)',
    ...
    check('JC-31 finding 1: a CROSS-BOOK structural base is REFUSED — the base must be a structural event of the SAME book',
    ...
    check('JC-31 finding 1: the rule is CLASS-level — book.add (non-root) and text.skeleton.set refuse a non-structural base identically',
    ...
    check('JC-31 finding 1: a legitimate re-add still chains to its book.remove — the unified rule adds no false refusal',
    ...
    check('JC-31 finding 1 (sibling): book.remove on an UNKNOWN base PENDS and the book STAYS PROJECTED — a removal never lands off an absent ancestor',
    ...
    check('JC-31 finding 1 (sibling): book.remove REFUSES a non-structural base and a CROSS-BOOK structural base — the most destructive op in the class is held to the same rule',
    ...
    check('JC-31 finding 1 (sibling): a well-formed book.remove chain still works — re-add projects generation 2 and the prior-generation decision is still quarantined (no regression)',
    ...
    check('JC-31: ASSERTED, not assumed — EVERY structural op (book.add, book.remove, text.skeleton.set, text.structure.apply) shares ONE base rule: unknown PENDS, non-structural REFUSES, cross-book REFUSES. No op is exempt, and a fifth would have to opt in here [covers R-8.5.2]',
    ...
    check('JC-31: audit — a CONTENT op with a cross-book base fails CLOSED: the head is excluded by ancestry and reported in retained[] as unselected-structural-branch; no text ever crosses books [covers R-8.6.8]',
  ...
  // --- FINDING 2: serializer/validator symmetry (see also the property test below) ---
  {
    ...
    check('JC-31 finding 2: every §5.2 identity component that would break serializer/validator symmetry is refused CLEANLY at seal AND at fold (delimiter-free, non-empty, JSON-safe, I-2 integer)',
  ...
  // --- FINDING 3 (SECURITY): a book code is a filesystem path — grammar + containment ---
  {
    ...
    check('JC-31 finding 3 (layer 1): a traversal-shaped book code is refused at seal AND at fold — every `book` field carries the §2 canonical grammar',
    ...
    check('JC-31 finding 3 (layer 1): a non-canonical or wrong-case book code is refused too — §2 says UPPERCASE eng-canon codes (D26)',
    ...
    check('JC-31 finding 3 (layer 2, defense in depth): WITH THE SCHEMA BYPASSED the checkpoint still refuses an escaping projection key — books AND decision sidecars; the fold\'s keys are never trusted (§2/§8.7) [covers R-8.7.6]',
  ...
  // --- FINDING 4 (SECURITY): a dotted path is a write target — grammar + null prototypes ---
  {
    ...
    check('JC-31 finding 4 (layer 1): every prototype-chain or empty-segment dotted path is refused at seal AND at fold, for settings.set AND project.meta.set (ONE dotted-path grammar) [covers R-8.5.8]',
    ...
    check('JC-31 finding 4 (layer 2, defense in depth): WITH THE SCHEMA BYPASSED the projection setters still cannot pollute — own-property traversal into null-prototype containers; ({}).tc4Polluted stays undefined [covers R-8.5.8]',
    ...
    check('JC-31 finding 4: the legitimate dotted-path overlay is unchanged by the hardening',
  ...
  // --- FINDING 5: scope ranges and initialVerses keys are grammars, not free strings ---
  {
    ...
    check('JC-31 finding 5: every §3 rule 4 scope violation and every non-slot initialVerses key is refused CLEANLY at seal AND at fold',
      allClean, details.join(' · ') || `${rows.length} firing cases`);
    ...
    check('JC-31 finding 5: every legal §3 rule 4 form (C, C-C, C:V, C:V-V, C:V-C:V, [] and multi-range) still seals and folds',
  ...
  // --- the CLASS-level guarantee: anything the writer seals survives JSON and its own reader ---
  {
    ...
    let accepted = 0;
    prop('JC-31 PROPERTY (the class-level guarantee, permanent): for every SCHEMA-VALID event — (a) its serialized §5.2 identity key validates, and (b) validateSegment(sealAction([e])) accepts. Anything the writer seals survives the JSON round trip and its own reader.',
      genEvent, (e) => {
    ...
    check('JC-31 PROPERTY: the generator actually reaches the accepting branch (the property is not vacuous)',
  ...
  //     other verse key carries, at every op that accepts a skeleton. ---
  {
    ...
    check('JC-31 finding 6: every SLOT KEY a skeleton derives carries the §8.4 slot grammar — at book.add, text.skeleton.set AND text.structure.apply. A `__proto__` slot used to seal, recompose to "[object Object]" and destroy the verse permanently [covers R-8.4.4]',
    ...
    check('JC-31 finding 6: the measured consequence is gone — the fold no longer produces `\\v 1 [object Object]` for a __proto__ slot; it refuses the event',
    ...
    check('JC-31 finding 6: DUPLICATE slot keys are refused too — two slots that name one verse head collapse silently at recompose [covers R-8.4.4]',
      allDup);
    check('JC-31 finding 6: a well-formed skeleton (single and span keys) still seals and folds — the hardening adds no false refusal',
  ...
  //     smuggled bytes lived OUTSIDE the I-3 validity hash. ---
  {
    ...
    check('JC-31 finding 7: journaled verse content carrying a §8.4 region marker (\\v /\\c ) or the reserved U+0001 is refused at seal AND at fold — at every op that carries verse content, so the committed book can never silently re-partition [covers R-8.4.3]',
      allClean, details.length ? `missed: ${details.join(' · ')}` : `${bad.length * 3} firing cases`);
    check('JC-31 finding 7: the codec and the grammar share ONE boundary definition — the schema refuses exactly the marker `decompose` splits on, so a slot the schema accepts always survives a decompose/recompose round trip',
      (() => {
        const add = mkEvent({ op: 'book.add', actor: 'actor-a', ts: okTs(0), book: 'TIT', scope: [], skeleton: skel('TIT'), initialVerses: {} });
  ...
  //     resources.json verbatim. ---
  {
    ...
    check('JC-31 finding 8: every §5.3 pin ENTRY is validated by the §5.3 entry shape — the slot grammar alone let `"not-an-object"` and `42` reach the projected resources.json',
      allClean, details.length ? `missed: ${details.join(' · ')}` : `${rows.length} firing cases`);
    check('JC-31 finding 8: ONE validator, shared — the same pinEntryError accepts every entry of the sample burrito\'s own §5.3 document',
      (() => {
  ...
  //     its own validator then accepted. ---
  {
    ...
    check('JC-31 finding 9: identityKeyOf validates its own components and THROWS — the serializer can no longer emit a string its own validator would accept from input the schema rejects (-0, arrays, objects, booleans, a missing reference)',
      laundered.length === 0, laundered.length ? `still laundered: ${laundered.map(([l]) => l).join(', ')}` : `${rows.length} firing cases`);
    check('JC-31 finding 9: a well-formed contextId still serializes — the precondition adds no false refusal',
  ...
  //     NaN, silently no-opped, and left the local clock PERMANENTLY behind. ---
  {
    ...
    check('JC-31 finding 10: an §8.2 ts MUST carry a real calendar instant, not merely the ISO charset — every non-calendar ts is refused at seal AND at fold [covers R-8.2.1]',
      allClean, details.length ? `missed: ${details.join(' · ')}` : `${rows.length} firing cases`);
    // the property the fix buys: string order IS instant order over the accepted set
    prop('JC-31 finding 10 PROPERTY: over every ts the grammar accepts, compareTs (string order) AGREES with parseTs (instant order) — the two orders can no longer disagree [covers R-8.2.1]',
      fc.record({
    ...
    check('JC-31 finding 10: real instants — including a genuine leap day — still seal and fold',
  ...
  //     creation/seed segment". The sentence was normative and unenforced. ---
  {
    ...
    let allClean = true; const details = [];
    for (const [label, ev] of rows) if (!refusedBothWays(ev).ok) { allClean = false; details.push(label); }
    check('JC-31 finding 11: project.vrs.set outside a creation/seed segment is refused at seal AND at fold — the §8.5 "creation/seed only" sentence now has an implementation [covers R-8.5.19]',
      allClean, details.length ? `missed: ${details.join(' · ')}` : `${rows.length} firing cases`);
    check('JC-31 finding 11: each legitimate seeding source still seals — creation, sidecar-migration and tc3-import',
  ...
  //     that silently loses a member is the one thing it may not be. ---
  {
    ...
    check('JC-31 finding 13: the §8.7 regeneration set is a NULL-PROTOTYPE container — a plain `{}` silently drops a `__proto__` key, so an "exhaustive" set could lose a member with no error anywhere',
    ...
    check('JC-31 finding 13: a `__proto__` projection key is RETAINED in the set and reported by the divergence classifier — pre-fix it vanished between emit and classification',
    ...
    check('JC-31 finding 13: classifyDivergence tests OWN membership, never `in` — `in` walks the prototype chain, so the "absent on disk is divergence too" rule would be read off a set that does not contain the key',
  ...
  // --- §8.1 actor.json: the slug is a directory name, createdAt is required ---
  {
    ...
    check('JC-31: validateActorDoc enforces the §8.1 actor-slug grammar (the actorId IS a directory name), a REQUIRED createdAt, and the types of the optional metadata fields [covers R-8.1.11 R-8.1.13]',
    ...
    check('JC-31: a complete, well-formed actor.json still validates (the hardening adds no false rejection)',
      validateActorDoc(doc({ displayName: 'A', device: 'laptop' }), 'actor-a').ok);
  }
}

// ---------- JC-32 (round 9, semantics half): `base` ABSENCE is a decided state, and no
...
//   unvalidated was the one the proof did not test. The matrix below is 4×4. ----------
{
  ...
  // --- the 4×4 matrix: EVERY structural op × EVERY base kind, `base: null` INCLUDED ---
  {
    ...
    // fold: an identical rootless book.add CONVERGES (a differing one forks — JC-32e);
    ...
    check('JC-32: `book.add` is the ONE rootless structural op, and only while the book does not exist — the first add still applies with no base (no false refusal)',
    ...
    check('JC-32: a ROOTLESS book.remove no longer DELETES the book — pre-fix it applied, unvalidated, and the book left the projection; per D53d it refuses to ACT, is retained and reported, and the fold completes',
    ...
    check('JC-32: the production path is closed too — §8.8 reconcile no longer emits a rootless structural event for a book the journal does not project; it emits the §8.8 seed `book.add` instead',
      (() => {
        const out = fold([add]);
  ...
  // --- the CONTENT half of the same rule ---
  {
    ...
    check('JC-32: a rootless CONTENT write no longer projects under EVERY structural branch — pre-fix `sanc == null` made `inChain` pass unconditionally, so it overwrote the winning branch\'s post-image. It is now retained and reported (`rootless-base`), which is exactly the sentence this PR added to §8.5 ("content ops therefore fail closed") finally being true [covers R-8.5.15]',
    ...
    check('JC-32: an UNRESOLVABLE base (present in no segment of the union) resolves to NO anchor, never to "every branch" — the head is excluded from the projection and reported',
    ...
    check('JC-32: generation membership for TEXT is CAUSAL, never an HLC cutoff — a still-offline actor\'s gen-1 edit with an arbitrarily LATER ts is quarantined by its ANCESTRY. Pre-fix the field-less branch ended in `h.ts > genRoot`, the exact mechanism §8.5 forbids by name [covers R-8.5.6]',
    ...
    check('JC-32: THE OFFLINE FORK STILL WORKS — two genuine concurrent FIRST writes (both rootless, both with no prior head) still fork and are both surfaced. The rule decides the rootless CLAIM, it does not outlaw rootless writes',
    ...
    check('JC-32: the rootless first write of every ancestry-free surface (pins, project metadata, settings) is untouched — those registers carry no structural branch at all',
      (() => {
...
//   JC-32b (the single-journal property), JC-32e (the two-seed convergence repro) and JC-32f
...
// ---------- JC-32b (round 9): SILENT DATA LOSS — the master invariant. Adversary D's
...
//   ----------
{
  ...
  // --- D-F1: a `supersedes` erased a live head with no ancestry check and no bookkeeping ---
  {
    ...
    check('JC-32b (D-F1): a SUPERSEDED head is CONSERVED — it is reported in retained[] as `superseded`. Pre-fix it could appear in NO list at all (retained[] is built from SURVIVING heads), so `\\v 1 ___` was committed over drafted text and the draft left the repo AND every review surface at once [covers R-8.3.6]',
  ...
  // --- D-F2: verse TEXT on a removed slot was the one dependent class with no guard ---
  {
    ...
    check('JC-32b (D-F2): with the disposition stated the action applies AND the dropped draft is conserved — retained, reported, reviewable',
    ...
    check('JC-32b (D-F2): no ZOMBIE when the slot returns — the dropped head is consumed on this branch, so a later structural action that re-creates the slot projects the stub, not a resurrected draft',
    ...
    check('JC-32b (D-F2): the ORPHAN BACKSTOP for text, equivalent to §8.6\'s alignment rule — a live text head on a key with NO SLOT in the current skeleton is reported, whatever produced it [covers R-8.6.6]',
      (() => {
  ...
  //  shared with JC-32e and JC-32f)
  {
    ...
    check('JC-32b CONSERVATION: the property is not vacuous — it exercised real records and real structural actions [covers R-8.6.2]',
    ...
    check('JC-32b CONSERVATION: the generator REACHES book removal and re-add — without these states the property could never see the F2 loss (a removed book\'s verse text, and a same-actor re-add erasure)',
    ...
    prop('JC-32b EXCLUSIVITY: a record is never PROJECTED and RETAINED at once — retained means "held for review, not in the projection". Pre-fix a note dispositioned `orphan-review` was both, pointing at a slot that was gone [covers R-8.6.3]',
      cmdArb, (cmds) => {
        const events = buildStream(cmds);
        const out = fold(events);
        for (const e of events) {
    ...
    prop('JC-32b DETERMINISM + IDEMPOTENCE + SUBSET-DETERMINISM + PARTIAL ARRIVAL: the fold is a pure function of the event SET — permutation, duplication, and any prefix of the stream all fold deterministically, and a partial union never projects a ts it does not contain',
      cmdArb, (cmds) => {
        const events = buildStream(cmds);
...
// ---------- JC-32c (round 9): the remaining semantics findings, each with its own firing
...
//   checkpoint's mandatory inputs, and the deferred layer-2 guards. ----------
{
  ...
  // --- D-F4: an ABSENT book folds but does not project — and its quarantine stays on ---
  {
    ...
    check('JC-32c (D-F4, the checkpoint consequence): the removed book no longer gets a §5.2 decision sidecar for a book outside `currentScope` — pre-fix the checkpoint either emitted one or threw for a missing resolution record [covers R-8.6.5]',
    ...
    check('JC-32c (D-F4): and the records come back under review, not into the projection — after a re-add they are gen-1 records against a gen-2 root, so they quarantine',
  ...
  // --- E-R10: fork identity is the op's §8.5 PAYLOAD, not "everything but the envelope" ---
  {
    ...
    const out = fold([add, a, b]);
    check('JC-32c (E-R10): an additive-optional field never manufactures a fork — §9 says readers MUST tolerate one without a version bump, and `payloadOf` built identity by SUBTRACTING the eight known envelope keys, so any other top-level field counted as identity and two IDENTICAL heads FORKED instead of auto-merging [covers R-8.3.9 R-8.6.4]',
      out.forks.length === 0 && out.books.TIT.verses['1:1'] === 'same\n', JSON.stringify(out.forks));
    check('JC-32c (E-R10): a genuine payload difference still forks — the fix narrows identity, it does not remove it',
      fold([add, a, { ...b, text: 'otro\n' }]).forks.length === 1);
    check('JC-32c (E-R10): `align.verse.set` is the deliberate exception — its payload IS the open §5.1 record spread at the top level, so an unknown field there is RECORD CONTENT and two heads that differ in it are genuinely different records',
      (() => {
    ...
    check('JC-32c (E-R10): `batch` never affects fold state even on the OPEN-payload op — two `align.verse.set` heads identical in every §5.1 field and differing ONLY in `batch` AUTO-MERGE. Pre-fix hazard (found by mutation audit M7/F1): dropping `batch` from the fold ENVELOPE subtraction silently joined it to align fork identity, so one section save per device manufactured a phantom fork per aligned verse — and no check could see it, because the only `batch` check exercised `text.verse.set`, whose additive payload row excludes envelope fields by construction [covers R-8.3.7]',
      (() => {
        const g = { generation: add.ts, book: 'TIT', chapter: '1', verse: '1', alignments: [], wordBank: [], targetVerseMd5: verseTextMd5('___\n') };
  ...
  // --- E-R9: a note names a DECISION, and a decision is (toolId, §5.2 identity) ---
  {
    ...
    check('JC-32c (E-R9): a note\'s `decisionKey` is the TOOLID-PREFIXED decision key — the same string the fold\'s `dec|` registers carry and disposition keys name. A bare five-part §5.2 identity key names a check POSITION, which two tools may both hold, so it could not say WHICH decision the note annotates',
    ...
    check('JC-32c (E-R9): the generation quarantine still reads the bookId out of the key — one grammar, one parse position',
      (() => {
  ...
  // --- E-Sweep1 #6/#7/#8: the checkpoint's mandatory inputs, completed ---
  {
    ...
    check('JC-32c (E-Sweep1 #6): the §8.7 mandatory-input guard covers the VERSIFICATION FRAME too. `vrs.json` was the one member of the "exhaustive" regeneration set emitted CONDITIONALLY (`if (foldOut.vrs)`) with no guard, so a journal with no `project.vrs.set` shipped a silently smaller checkpoint [covers R-8.7.4]',
    ...
    check('JC-32c (E-Sweep1 #7): and divergence detection therefore LISTS vrs.json — the enumeration starts from the fold\'s expected set, so a vrs.json deleted out of band is divergence like every other derived file (§8.8) [covers R-8.7.1 R-8.7.5]',
  ...
  {
    {
      ...
      check('JC-32c (deferred): the §5.1 extraction (I-3) REFUSES content it cannot cover, instead of returning a hash over PART of it — the same ONE §8.4 boundary grammar the schema applies, applied again at the hash [covers R-8.5.10]',
        errs.every((e) => e.includes('ONE content slot')), JSON.stringify(errs.map((e) => e.slice(0, 45))));
    }
    check('JC-32c (deferred): ordinary content still hashes as before — the widened extraction changes no existing hash',
    ...
    check('JC-32c (deferred, layer 2): head IDENTITY is built with own-key writes. `p[k] = e[k]` runs the PROTOTYPE setter for a `__proto__` key and SWALLOWS the field, so fork detection went blind on it — two different records compared equal and auto-merged',
    ...
    check('JC-32c (deferred, layer 2): the canonicalization walk is BOUNDED like every other recursive consumer — the schema bounds depth at §8.1, and this walk refuses it AGAIN so a document reaching the fold with validation off gets a verdict, never a stack overflow',
      deepErr.includes('§8.1') && !/Maximum call stack/.test(deepErr), `"${deepErr.slice(0, 70)}"`);
    {
      ...
      check('JC-32c (deferred, #92): canonical identity keeps JSON.stringify\'s OWN-PROPERTY key order — integer-index keys ascend numerically before lexical string keys ("2" before "10"; "01" and 2^32-1 are NOT indices)',
      ...
      check('JC-32c (deferred, #92, property): the direct serializer equals the old rebuild-then-stringify BYTE FOR BYTE over generated documents (integer-like keys, nesting, arrays, undefined, __proto__)',
  ...
  // --- B-F5: the prefix-collision rule, normative [§8.5, D54, decided 2026-08-17] ---
  {
    ...
    check('JC-32b/B-F5: the same rule binds project metadata, and unrelated sibling paths are untouched',
      (() => {
...
// ---------- JC-32d (s8 round 10): verse TEXT of a REMOVED book joins the conservation
...
//   head before the quarantine could see it. ----------
{
  ...
  check('JC-32d (F2): a removed book\'s live verse-text heads are RETAINED and reported (`absent-book`) — the same conservation rule decisions, alignments and notes already had. Pre-fix the draft was in NO observable state: not projected, not retained, not invalid, not pending, not forked [covers R-8.6.5]',
  ...
  check('JC-32d (F2, the worse variant): a SAME-ACTOR remove + re-add quarantines the prior-generation text head (`prior-generation`) exactly as a different actor\'s would — pre-fix the same-actor linear rule consumed the prior-generation head and the draft was ERASED with no report [covers R-8.6.5]',
  ...
  check('JC-32d (F2): the DIFFERENT-actor re-add gives the same verdict — one rule, no actor-dependent data loss',
...
// ---------- JC-32e (s8 round 10, D53 part d): `base: null` asserts "no prior state I KNOW
...
//   per-event (retained, reported) — never a whole-fold throw. ----------
{
  ...
  // (a) the two-device seeding union CONVERGES — one book, no fork, both orders
  {
    ...
    check('JC-32e (D53d): the convergence is DETERMINISTIC under order and permutation — the fold stays a pure function of the event SET [covers R-8.6.1 R-8.8.3]',
    ...
    check('JC-32e (R-8.6.2): the exact two-seed repro passes the MASTER conservation predicate — every written record of BOTH actors ends in an observable state, the aliased creation included [covers R-8.6.2 R-8.6.4]',
    ...
    check('JC-32e (R-8.6.4): THREE identical seeds collapse to ONE autoMerged entry carrying the COMPLETE head-ts set — never one pairwise entry per alias — identical in every order, and every seed event of all three actors stays observable [covers R-8.6.4 R-8.6.2]',
  ...
  // (b) a DIFFERENT-payload rootless book.add FORKS and surfaces — neither silently wins
  {
    ...
    check('JC-32e (D53d): two rootless book.add with DIFFERENT payloads FORK and surface for review like any structural fork — neither silently wins, and the whole fold is never refused [covers R-8.5.3]',
  ...
  // (c) a rootless book.remove / structure op on an existing book is REPORTED, never a throw
  {
    ...
    check('JC-32e (D53d): a rootless book.remove on an existing book still refuses to ACT (the book stays projected) but is RETAINED and REPORTED (`rootless-structural`) — the project folds; pre-fix the whole fold threw [covers R-8.3.4 R-8.5.3]',
    ...
    check('JC-32e (D53d): a rootless text.structure.apply is the same verdict — it never fires blind (the slot set is unchanged) and never refuses the whole fold; it is retained and reported [covers R-8.3.4 R-8.5.3]',
    ...
    check('JC-32e (D53d): and text.skeleton.set — the whole rootless-structural class is one rule: refuse to act, report, keep folding [covers R-8.3.4 R-8.5.3]',
...
// ---------- JC-32f (round 12): the same-actor stale-base rule refuses DESCENT, not
...
//   was permanently unfoldable for every writer. ----------
{
  ...
  // (a) a D53d differing-add fork, then BOTH actors restructure off the fork winner
  {
    ...
    check('JC-32f: after a D53d differing-add fork, an honest union where both actors restructured off the fork WINNER folds — actor A\'s losing rootless genesis is a live head of actor A, but it never ADVANCED PAST the base, so the stale-own-head rule stays silent and the concurrent applies surface as an ordinary structural fork. Pre-fix the union THREW (staleOwnSkeletonHead read ANY same-actor live head as "my own head advanced past my base") while each journal folded clean alone [covers R-8.5.14]',
      aloneOk && err === '' && !!out &&
  ...
  // (a2) the retention REASON of a draft on the fork-LOSING genesis is a branch miss
  {
    ...
    check('JC-32f: a draft anchored to the fork-LOSING genesis is retained as `unselected-structural-branch` — a fork-losing (or converged) add was never this book\'s generation root, so it is not a PRIOR root. Pre-fix rootsOfBook counted every book.add in the union and the reason mis-reported as `prior-generation`',
  ...
  //     the fork winner — same shape, no rootless second add involved
  {
    ...
    check('JC-32f: the same union shape WITHOUT the D53d path — a same-base skeleton fork, then both actors restructure off the fork winner — folds too: actor A\'s fork-loser skeleton head does not descend from the base, so the refusal does not fire. The ORIGINAL E-R3 refusal (a base the actor\'s own head genuinely advanced past — JC-32c) is unchanged [covers R-8.5.14]',
  ...
  //     journals. The property: the union of two honestly-written journals ALWAYS folds.
  {
    ...
    check('JC-32f REACH (non-vacuity): across the runs the two-device generator REACHED at least one fork, one pending structural state, one converged-join auto-merge, and one book.remove — the states item 1\'s escape class hides in; a property that never visits them proves nothing [covers R-8.6.2]',
...
// ---------- JC-32g (round 12): the D53d alias covers EVERY comparison point. After
...
//   structural actions pended `conflicted` forever with their drafts retained forever. ----------
{
  ...
  //     slot-head ts (the aliased root). Linear advance, NO fork.
  {
    ...
    check('JC-32g: a post-convergence verse edit whose base names the ALIASED seed root\'s slot head advances LINEARLY — no fork, the edit projects, one live head. Pre-fix joinHead compared the base ts literally, missed the canonical slot head, and raised a PHANTOM FORK on every verse the re-seeded device touched [covers R-8.5.3 R-8.8.3]',
  ...
  //     and text.skeleton.set both advance the canonical lineage — no phantom fork.
  {
    ...
    check('JC-32g: a post-convergence book.remove and a slot-preserving text.skeleton.set chained to the ALIASED root are ordinary chain links of the canonical lineage — the remove removes (no fork), the skeleton edit advances the skel head (no fork, nothing pending) [covers R-8.5.3 R-8.8.3]',
...
// ---------- JC-32h (round 12): an auto-merge LOSING TWIN is ACCOUNTED. R-8.6.4 collapses
...
//   the winning ts): the twin's observable state IS the projected identical head. ----------
{
  ...
  // (b) the plain same-root variant — no alias involved, both stamps literally equal
  {
    ...
    check('JC-32h: the plain same-root identical twins are reported the same way — auto-merge bookkeeping is one rule, alias or no alias [covers R-8.6.4]',
...
// ---------- JC-33: the OBS story kind — journal vocabulary version 2 (BURRITO-SPEC §10, D74) ----------
// The file-level checks (schema, layout, frame model, byte-strict write, scope, the v2 fold
// to bytes) are the `OBS` group of validate.mjs. This group proves the JOURNAL semantics of
// §10: frame registers, story targets, the §5.2 story key, the §10.6 pin slots, and the
// checkpoint's story inputs. Every check fires on its negative.
{
  const A = 'actor-a', B = 'actor-b';
  const t = (s, c, a) => `2026-09-15T14:00:${String(s).padStart(2, '0')}.000Z|000${c}|${a}`;
  ...
  // Same-actor edits advance linearly; two actors' rootless first writes on ONE frame fork
  // (surfaced, provisional winner) unless identical (auto-merged, accounted); a base that
  // names the live head is a linear supersede; frame 0 is the title's own register.
  {
    const e1 = ev({ op: 'text.frame.set', actor: A, ts: t(1, 0, A), story: 1, frame: 1, text: 'uno' });
    const e2 = ev({ op: 'text.frame.set', actor: A, ts: t(2, 0, A), story: 1, frame: 1, text: 'uno bis' }); // same actor, rootless: linear (R-8.3.3)
    const e3 = ev({ op: 'text.frame.set', actor: B, ts: t(3, 0, B), story: 1, frame: 1, text: 'otro' });     // other actor, rootless: FORK
    const e4 = ev({ op: 'text.frame.set', actor: B, ts: t(4, 0, B), story: 1, frame: 2, text: 'dos' });
    const e5 = ev({ op: 'text.frame.set', actor: A, ts: t(5, 0, A), story: 1, frame: 0, text: 'Título' });
    const linear = fold([e1, e2]);
    const forked = fold([e1, e2, e3, e4, e5]);
    const resolved = fold([e1, e2, e3, e4, e5, ev({ op: 'text.frame.set', actor: A, ts: t(6, 0, A), story: 1, frame: 1, text: 'final', base: t(2, 0, A), supersedes: [t(3, 0, B)] })]);
    const twin = fold([e1, ev({ op: 'text.frame.set', actor: B, ts: t(7, 0, B), story: 1, frame: 1, text: 'uno' })]);
    const okLinear = linear.stories[1].frames[1] === 'uno bis' && linear.forks.length === 0;
    const okFork = forked.forks.length === 1 && forked.forks[0].key === 'frame|1|1' && forked.stories[1].frames[1] === 'otro' && forked.forks[0].provisional === t(3, 0, B) &&
      forked.stories[1].frames[2] === 'dos' && forked.stories[1].frames[0] === 'Título' && forked.stories[1].ref === null;
    const okResolved = resolved.forks.length === 0 && resolved.stories[1].frames[1] === 'final' && resolved.retained.some((r) => r.key === 'frame|1|1' && r.ts === t(3, 0, B) && r.reason === 'superseded');
    const okTwin = twin.forks.length === 0 && twin.autoMerged.some((m) => m.key === 'frame|1|1') && twin.stories[1].frames[1] === 'uno';
    // no generation, no chain: a frame write is never `rootless-base`, never `prior-generation`
    const noQuarantine = !forked.retained.some((r) => r.key.startsWith('frame|'));
    // negative: the SAME rootless two-actor scenario on a verse slot of a book is the
    // `rootless-base` writer defect (R-8.5.15) — the frame register deliberately is not
    const skeleton = `\\id TIT\n\\c 1\n\\v 1 ${SLOT}1:1${SLOT}`;
    const bookAdd = { v: 1, base: null, op: 'book.add', actor: A, ts: t(8, 0, A), book: 'TIT', scope: [], skeleton, initialVerses: {} };
    const verseRootless = fold([bookAdd,
      { v: 1, base: null, op: 'text.verse.set', actor: A, ts: t(9, 0, A), book: 'TIT', chapter: '1', verse: '1', text: 'a\n' },
      { v: 1, base: null, op: 'text.verse.set', actor: B, ts: t(10, 0, B), book: 'TIT', chapter: '1', verse: '1', text: 'b\n' }]);
    const fires = verseRootless.retained.some((r) => r.reason === 'rootless-base');
    check('JC-33a: frame registers — one LWW register per frame (frame 0 = the title) and per reference line, NO structural chain and NO generation: same-actor rootless edits advance linearly, two actors\' rootless first writes on one frame FORK (surfaced, provisional = highest ts), a supersedes resolves it (loser retained `superseded`), identical twins auto-merge and are accounted, and no frame head is ever quarantined; the same rootless two-writer scenario on a BOOK verse is the rootless-base defect, so the difference is deliberate [covers R-10.7.2 R-10.4.1]',
      okLinear && okFork && okResolved && okTwin && noQuarantine && fires,
      `fork ${JSON.stringify(forked.forks[0]?.heads)} → provisional ${forked.forks[0]?.provisional?.slice(0, 19)}`);
  }

  // JC-33b — story targets: a check.decision.set whose reference is {story, frame} and a
  // note.add whose target is {story, frame} carry NO generation, need v: 2, key by the
  // five-part identity with the literal `obs`, and project — the decision into
  // checking/<toolId>/OBS.json, the note with its frame target intact.
  {
    const dec = (i, actor, text, extra = {}) => ev({ op: 'check.decision.set', actor, ts: t(i, 0, actor), toolId: 'translationNotes',
      decision: { contextId: { checkId: 'tn01', occurrence: 1, reference: { story: 1, frame: 1 } }, selections: false, note: text }, ...extra });
    const note = (i, actor, target, extra = {}) => ev({ op: 'note.add', actor, ts: t(i, 0, actor), target, text: 'comentario', ...extra });
    const out = fold([dec(1, A, 'first'), dec(2, A, 'second'), note(3, A, { story: 1, frame: 1 }), note(4, A, { decisionKey: 'translationNotes|tn01|obs|1|1|1' })]);
    const d = out.decisions.translationNotes;
    const keyed = identityKeyOf(d[0].contextId) === 'tn01|obs|1|1|1' && identityKeyError('tn01|obs|1|1|1') === null && decisionKeyError('translationNotes|tn01|obs|1|1|1') === null;
    const projected = derivedProjections(out, { baseMetadata, resolutions: { translationNotes: { OBS: { repoPath: 'git.door43.org/unfoldingWord/en_obs-tn', sha: 'e86138ea13f619f09f7a6dcaa60592716d407fe4', version: 'v13', languageSet: 'fallback' } } } });
    const file = JSON.parse(projected['checking/translationNotes/OBS.json'] || 'null');
    const okDecision = d.length === 1 && d[0].note === 'second' && keyed && file && file.book === 'OBS' && file.decisions.length === 1 && file.decisions[0].contextId.reference.story === 1;
    const okNotes = out.notes.length === 2 && out.notes[0].target.story === 1 && out.notes[0].target.frame === 1 && out.notes[1].target.decisionKey === 'translationNotes|tn01|obs|1|1|1' && !out.retained.some((r) => r.key === 'note');
    // negatives, at the ONE schema (seal, intake and fold share it): a generation stamp on a
    // story target refuses; v: 1 refuses both story targets; a mixed reference refuses; a
  ...
  // grammar; they project after simplifiedText; an unknown OBS-looking slot refuses.
  {
    ...
    const bible = JSON.parse(fs.readFileSync(ING('checking/resources.json'), 'utf8'));
    const base = [];
    let i = 1;
  ...
  // onto the base story (mandatory), enumerated for divergence like every other; a frame
  // beyond the story's frame set refuses at checkpoint (no structural op can add one).
  {
    const out = fold([ev({ op: 'text.frame.set', actor: A, ts: t(1, 0, A), story: 1, frame: 2, text: 'dos' }), ev({ op: 'text.story.ref.set', actor: A, ts: t(2, 0, A), story: 1, text: 'Génesis 1-2' })]);
    const proj = derivedProjections(out, { baseMetadata, baseStories: { 1: seed01 } });
    const bytes = proj['content/01.md'];
    const parsed = storyMod.parseStory(bytes);
    const inSet = typeof bytes === 'string' && parsed.frames[1].text === 'dos' && parsed.ref === 'Génesis 1-2' && parsed.frames[0].text === '' && bytes.split('\n')[0] === '# 1.';
    // divergence: an out-of-band edit of the story file, and its deletion, are both detected
    const disk = { ...proj, 'content/01.md': bytes.replace('dos', 'DOS') };
    const dv = classifyDivergence(disk, proj);
    const deleted = { ...proj }; delete deleted['content/01.md'];
    const dv2 = classifyDivergence(deleted, proj);
    const divergence = dv.diverged.includes('content/01.md') && dv2.diverged.includes('content/01.md') && !dv.diverged.includes('metadata.json');
    // negatives: no base story → refuse; a frame the base story does not have → refuse;
    // a base whose frame set is the story's is the ONLY thing that decides the frame count
  ...
  // two story ops and the story targets are v: 2 only; v: 3 is unknown.
  {
    ...
    const v1Only = s.events.every((e) => e.v === 1) && before.books.TIT.usfm === s.books.TIT && Object.keys(before.stories).length === 0;
    const v2Frame = ev({ op: 'text.frame.set', actor: A, ts: t(1, 0, A), story: 1, frame: 1, text: 'x' });
    const v2Ref = ev({ op: 'text.story.ref.set', actor: A, ts: t(2, 0, A), story: 1, text: 'x' });
    const v2ok = validateEvent(v2Frame) === null && validateEvent(v2Ref) === null;
    const v1Refuses = /v: 2/.test(validateEvent({ ...v2Frame, v: 1 }) || '') && /v: 2/.test(validateEvent({ ...v2Ref, v: 1 }) || '');
    const v3Refuses = /version/.test(validateEvent({ ...v2Frame, v: 3 }) || '');
    // a v: 2 envelope on a v: 1 op is legal (v: 2 is a superset): a v: 2 verse edit based on
    // a v: 1 verse head continues that head without a fork and projects
    const skeleton = `\\id TIT\n\\c 1\n\\v 1 ${SLOT}1:1${SLOT}`;
    const add = { v: 1, base: null, op: 'book.add', actor: A, ts: t(6, 0, A), book: 'TIT', scope: [], skeleton, initialVerses: {} };
    const v1Edit = { v: 1, base: add.ts, op: 'text.verse.set', actor: A, ts: t(7, 0, A), book: 'TIT', chapter: '1', verse: '1', text: 'uno\n' };
    const v2Edit = { ...v1Edit, v: 2, ts: t(8, 0, A), base: v1Edit.ts, text: 'uno bis\n' };
    const mixed = fold([add, v1Edit, v2Edit]);
    const superset = mixed.forks.length === 0 && mixed.books.TIT.verses['1:1'] === 'uno bis\n';
    // the union of a v: 1 seed and the v: 2 story events folds: books AND stories, side by side
    const union = fold([...s.events, v2Frame, v2Ref]);
    const sideBySide = union.books.TIT.usfm === s.books.TIT && union.stories[1].frames[1] === 'x' && union.stories[1].ref === 'x';
