...
/** How a section's keys changed, as old→new groups joined by the verse
 * numbers they share: a create is {from: ["9","10"], to: ["9-10"]}, a break
 * the reverse. Keys present on both sides are not in any group. */
export const keyMapping = (oldKeys, newKeys) => {
  const groups = [];
  const place = (k, side) => {
    const nums = expandKeys([k]);
    ...
    g.nums.push(...nums);
    g[side].push(String(k));
  };
  for (const k of oldKeys) if (!newKeys.map(String).includes(String(k))) place(k, 'from');
  for (const k of newKeys) if (!oldKeys.map(String).includes(String(k))) place(k, 'to');
