 ...
 * story catalogue; frame markers only describe the currently open story. */
              ...
              {selected && story && (
                <div data-testid="story-frame-markers" style={{ display: 'flex', flexWrap: 'wrap', gap: 4, padding: '2px 8px 8px' }}>
                  {story.frames.map((_, index) => (
                    <span key={index + 1} title={t('storyDraft.frameLabel', { n: index + 1 })}
                      style={{ width: 22, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--radius-xs)', background: 'var(--surface-accent-soft)', color: 'var(--text-accent)', fontSize: 'var(--fs-label)', fontWeight: 'var(--fw-bold)' }}>
                      {index + 1}
                    </span>
                  ))}
                </div>
