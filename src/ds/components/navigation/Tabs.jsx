...
export function Tabs({ tabs = [], value, onChange, style }) {
  return (
    <div style={{ display: 'flex', gap: 0, padding: '0 14px', borderBottom: 'var(--stroke-hair) solid var(--border)', ...style }}>
      {tabs.map(t => {
        const v = typeof t === 'string' ? t : t.value;
        const on = v === value;
        return <button key={v} type="button" onClick={() => onChange && onChange(v)} style={{
