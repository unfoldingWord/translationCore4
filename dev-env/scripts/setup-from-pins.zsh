...
fetch_pinned() {  # $1 repo url, $2 dir, $3 rev
  ...
  # (#306). The clones are disposable and read-only, so refresh hard.
  if [ ! -d "$2/.git" ]; then git -c core.autocrlf=false clone --quiet "$1" "$2"; else git -C "$2" fetch --quiet origin; fi
