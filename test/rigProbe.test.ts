...
import { assertRigHealthy, RIG_HEALTH_TIMEOUT_MS, RIG_HEALTH_URL, type RigFetch } from '../e2e/rig-health';
...
describe('#324 — the journey rig health probe', () => {
  it('accepts HTTP 200 with one request and no output', async () => {
    ...
    expect(fetchFn.mock.calls).toEqual([[RIG_HEALTH_URL, { signal: expect.any(AbortSignal) }]]);
